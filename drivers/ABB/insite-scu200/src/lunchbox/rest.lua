...
function RestClient.one_shot_get(full_url, additional_headers, socket_builder)
  ...
  local query_params = ""
  if url_table.query ~= nil then
    query_params = "?"
    
    for param, value in pairs(url_table.query) do
      query_params = query_params .. param .. "=" .. value .. "&"
    end
    
    query_params = query_params:sub(1, -2)
  end

  local client = RestClient.new(url_table.scheme .. "://" .. url_table.authority, socket_builder)
  local ret, err = client:get(url_table.path .. query_params, additional_headers)
...
function RestClient.one_shot_post(full_url, body, additional_headers, socket_builder)
  ...
  local client = RestClient.new(url_table.scheme .. "://" .. url_table.authority, socket_builder)
  local ret, err = client:post(url_table.path .. query_params, body, additional_headers)
