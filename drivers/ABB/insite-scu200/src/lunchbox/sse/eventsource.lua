...
--- @param body table a set of key/value pairs (strings) to be sent as the body of the initial POST request.
local function create_request(url_table, extra_headers, body)
  local request = Request.new("POST", url_table.path, nil)
  ...
  request = request:append_body(encoded_body)
...
local function connecting_action(source)
  ...
  local request = create_request(source.url, source._extra_headers, source._body)
...
--- @param extra_headers table|nil an optional table of key-value pairs (strings) to be added to the initial POST request
--- @param body table|nil an optional table of key-value pairs (strings) to be sent as the body of the initial POST request
...
function EventSource.new(url, extra_headers, body, sock_builder)
  ...
  local source = setmetatable({
    ...
    _last_field = nil,
    _reconnect = true,
    _reconnect_time_millis = 15 * 1000,
  ...
  cosock.spawn(function()
    ...
    while true do
      if source.ready_state == EventSource.ReadyStates.CLOSED and not source._reconnect then
        return
      end
