...
function initiateSensors(driver, device)
  ...
  if string.find(device.device_network_id, controllerLabel .. "%[(.-)%]", 0, false) then
    ...
    local controllerStatus = deviceController:get_latest_state("main", connectionStatus.ID, "UDP", "no value", "no value")
    if controllerStatus == "Connected" then
      device:online()
    else
      device:offline()
    end
...
function startUDPCommunication()
  ...
  if UDPDebugMode == false then
    ...
    stable_delay()
    for i = 1, 8 do
      local portNumber = tostring("00" ..i)
      if deviceForPort[portNumber] ~= nil then 
        deviceForPort[portNumber]:offline()
        stable_delay()
      end
    end
  else
    ...
    for i = 1, 8 do
      local portNumber = tostring("00" ..i)
      if deviceForPort[portNumber] ~= nil then 
        deviceForPort[portNumber]:online()
        stable_delay()
      end
    end
  end
...
-- check X-talk address of data, so that events can be send to the correct device
 ...
 -- Lidar sensor
    ...
    -- Check if controller is still alive during opertation
    ...
    elseif data ~= nil and string.find(data, "N000B[IP=", 1, true) then
      ...
      for i = 1, 8 do
        ...
        if string.find(tostring(deviceForPort[xtalkportSet]), "Presence") then
          deviceForPort[xtalkportSet]:online()
        ...
        elseif string.find(tostring(deviceForPort[xtalkportSet]), "Lidar") then
          ...
          socket.sleep(0.5)
        elseif string.find(tostring(deviceForPort[xtalkportSet]), "RFID") then
          deviceForPort[xtalkportSet]:online()
        end
      end
      settingsSend = true
