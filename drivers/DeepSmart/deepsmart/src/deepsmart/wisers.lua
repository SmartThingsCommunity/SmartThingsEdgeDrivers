...
function Wisers.wiser_loop(wiser_index_code)
  ...
  -- get wiser
  -- query changed devs
  local res = wiser.loopapi:load_changeddevs(wiser.last_active_time)
  if (res ~= nil) then
...
function Wisers.reload(uuid, add_devs, del_devs)
  ...
  -- get devices
  local configstr = deepsmartapi:load_config()
  log.trace(string.format('load config (%s)', configstr))
  if (configstr ~= nil and configstr ~= wiser.bridge:get_field(config.FIELD.DEVICES)) then
  ...
  -- get dp info
  local dp2knx_config = deepsmartapi:load_dp2knx()
  if (dp2knx_config ~= nil and dp2knx_config ~= wiser.bridge:get_field(config.FIELD.DP2KNX)) then
  ...
  -- get enum info
  local dpenum_config = deepsmartapi:load_dpenum()
  if (dpenum_config ~= nil and dpenum_config ~= wiser.bridge:get_field(config.FIELD.DPENUM)) then
...
function Wisers.get_dev_type(device_network_id)
  local dev_id,_ = Wisers.parse_device_network_id(device_network_id)
  local wiser,dev = Wisers.get_device_wiser(dev_id)
  if (wiser == nil) then
    -- when device is not exist in bridge delete from hub
    log.warn('device '..device_network_id..' is not exist for read')
    Wisers.del_device_by_id(dev_id)
