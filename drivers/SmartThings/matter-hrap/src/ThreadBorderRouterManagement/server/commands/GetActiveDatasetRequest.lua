...
GetActiveDatasetRequest.NAME = "GetActiveDatasetRequest"
GetActiveDatasetRequest.ID = 0x0000
GetActiveDatasetRequest.field_defs = {
}

function GetActiveDatasetRequest:init(device, endpoint_id)
  local out = {}
  local args = {}
  ...
  setmetatable(out, {
    __index = GetActiveDatasetRequest,
    __tostring = GetActiveDatasetRequest.pretty_print
  })
...
function GetActiveDatasetRequest:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

function GetActiveDatasetRequest:augment_type(base_type_obj)
  local elems = {}
...
function GetActiveDatasetRequest:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(GetActiveDatasetRequest, {__call = GetActiveDatasetRequest.init})
