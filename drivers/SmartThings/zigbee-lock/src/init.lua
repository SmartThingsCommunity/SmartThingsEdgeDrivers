...
local refresh = function(driver, device, cmd)
  ...
  if not device:supports_capability_by_id(LockCodes.ID) and not device:get_field(legacy_lock_utils.CHECKED_CODE_SUPPORT) then
    ...
    device:set_field(legacy_lock_utils.CHECKED_CODE_SUPPORT, true)
  end
end
...
local do_configure = function(self, device)
  ...
  if slga_migrated then
    ...
    if device.data.lockCodes == nil or device:get_field(legacy_lock_utils.MIGRATION_RELOAD_SKIPPED) == true then
      ...
      device:set_field(legacy_lock_utils.MIGRATION_RELOAD_SKIPPED, true, { persist = true })
    end
  end
end
...
local programming_event_handler = function(driver, device, zb_mess)
  ...
  elseif (zb_mess.body.zcl_body.program_event_code.value == ProgrammingEventCodeEnum.PIN_CODE_DELETED) then
    if (credential_index == lock_utils.DELETE_ALL_USERS) then
