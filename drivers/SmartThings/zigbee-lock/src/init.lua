...
-- Shared handlers (apply to all devices) --
...
-- Default (post-migration) handlers --
...
local reload_all_codes = function(device)
  ...
  if (device:get_field(new_lock_utils.CHECKING_CODE) == nil) then
    device:set_field(new_lock_utils.CHECKING_CODE, 1)
  end

  device:send(LockCluster.server.commands.GetPINCode(device, device:get_field(new_lock_utils.CHECKING_CODE)))
end
...
local add_user_handler = function(driver, device, command)
  if new_lock_utils.busy_check_and_set(device, {name = new_lock_utils.ADD_USER, type = new_lock_utils.LOCK_USERS}) then
    return
  end
  ...
  if available_index == nil then
    ...
    device:set_field(new_lock_utils.ACTIVE_CREDENTIAL, { userIndex = available_index})
    new_lock_utils.create_user(device, command.args.userName, command.args.userType, available_index)
  end

  if status == new_lock_utils.STATUS_SUCCESS then
    new_lock_utils.send_events(device, new_lock_utils.LOCK_USERS)
  end
...
local update_user_handler = function(driver, device, command)
  if new_lock_utils.busy_check_and_set(device, {name = new_lock_utils.UPDATE_USER, type = new_lock_utils.LOCK_USERS}) then
    return
  end
  ...
  local status = new_lock_utils.STATUS_FAILURE

  for _, user in pairs(current_users) do
    if user.userIndex == user_index then
      device:set_field(new_lock_utils.ACTIVE_CREDENTIAL, { userIndex = user_index})
      user.userName = user_name
      user.userType = user_type
      device:set_field(new_lock_utils.LOCK_USERS, current_users, { persist = true })
      new_lock_utils.send_events(device, new_lock_utils.LOCK_USERS)
      status = new_lock_utils.STATUS_SUCCESS
      break
    end
  end
...
local delete_user_handler = function(driver, device, command)
  if new_lock_utils.busy_check_and_set(device, {name = new_lock_utils.DELETE_USER, type = new_lock_utils.LOCK_USERS}, command.override_busy_check) then
    return
  end
  local status = new_lock_utils.STATUS_SUCCESS
  local user_index = tonumber(command.args.userIndex)
  if new_lock_utils.get_user(device, user_index) ~= nil then
    ...
    local associated_credential = new_lock_utils.get_credential_by_user_index(device, user_index)
    if associated_credential ~= nil then
      ...
      new_lock_utils.delete_user(device, user_index)
      new_lock_utils.send_events(device, new_lock_utils.LOCK_USERS)
      new_lock_utils.clear_busy_state(device, status, command.override_busy_check)
    end
  else
...
local delete_all_users_handler = function(driver, device, command)
  if new_lock_utils.busy_check_and_set(device, {name = new_lock_utils.DELETE_ALL_USERS, type = new_lock_utils.LOCK_USERS}) then
    return
  end
  local status = new_lock_utils.STATUS_SUCCESS
  local current_users = new_lock_utils.get_users(device)
...
local update_credential_handler = function(driver, device, command)
  if new_lock_utils.busy_check_and_set(device, {name = new_lock_utils.UPDATE_CREDENTIAL, type = new_lock_utils.LOCK_CREDENTIALS}) then
    return
  end
  ...
  if credential ~= nil then
    ...
    new_lock_utils.clear_busy_state(device, new_lock_utils.STATUS_FAILURE)
  end
end

local delete_credential_handler = function(driver, device, command)
  if new_lock_utils.busy_check_and_set(device, {name = new_lock_utils.DELETE_CREDENTIAL, type = new_lock_utils.LOCK_CREDENTIALS}, command.override_busy_check) then
    return
  end
  ...
  if credential ~= nil then
    ...
    new_lock_utils.clear_busy_state(device, new_lock_utils.STATUS_FAILURE, command.override_busy_check)
  end
end

local delete_all_credentials_handler = function(driver, device, command)
  if new_lock_utils.busy_check_and_set(device, {name = new_lock_utils.DELETE_ALL_CREDENTIALS, type = new_lock_utils.LOCK_CREDENTIALS}) then
    return
  end
...
local get_pin_response_handler = function(driver, device, zb_mess)
  ...
  elseif zb_mess.body.zcl_body.user_status.value == UserStatusEnum.AVAILABLE and command ~= nil and command.name == new_lock_utils.ADD_CREDENTIAL then
...
local lock_operation_event_handler = function(driver, device, zb_rx)
  ...
  if (event ~= nil) then
    ...
    if (source == 0 and device:supports_capability_by_id(capabilities.lockUsers.ID)) then --keypad
      ...
      local user = new_lock_utils.get_user(device, code_id)
      if user ~= nil then
...
local zigbee_lock_driver = {
  ...
  capability_handlers = {
    [Lock.ID] = {
      [Lock.commands.lock.NAME] = handle_lock,
      [Lock.commands.unlock.NAME] = handle_unlock,
    },
