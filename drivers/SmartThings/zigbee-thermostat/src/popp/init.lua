...
local supported_thermostat_modes_handler = function(driver, device, supported_modes)
  device:emit_event(ThermostatMode.supportedThermostatModes({ ThermostatMode.thermostatMode.heat.NAME }))
end
...
local do_configure = function(driver, device)
  ...
  device:send(PowerConfiguration.attributes.BatteryVoltage:configure_reporting(device, 1, 21600, 1))
  device:send(PowerConfiguration.attributes.BatteryPercentageRemaining:configure_reporting(device, 3600, 43200, 2))
...
local info_changed = function(driver, device, event, args)
    ...
    for name, info in pairs(common.PREFERENCE_TABLES) do
        ...
        if (device.preferences[preferenceId] ~= nil and args.old_st_store.preferences[preferenceId] ~= device.preferences[preferenceId]) then
            local input = device.preferences[preferenceId]
            
            -- set keypad lock (child lock)
            if (name == common.KEYPAD_LOCK) then
              device:send(ThermostatUIConfig.attributes.KeypadLockout:write(device, tonumber(input)))
            -- set viewing direction (0° or 180°)
            elseif (name == common.VIEWING_DIRECTION) then
              device:send(cluster_base.write_manufacturer_specific_attribute(device, info.clusterId, info.attributeId, common.MFG_CODE, info.dataType, tonumber(input)))
            -- set orientation (vertical true/false)
            elseif (name == common.ETRV_ORIENTATION) then
              device:send(cluster_base.write_manufacturer_specific_attribute(device, info.clusterId, info.attributeId, common.MFG_CODE, info.dataType, input))
            -- set regulation setpoint offset (-2.5 to 2.5) 
            elseif (name == common.REGUALTION_SETPOINT_OFFSET) then
              local offset = tonumber(input) * 10 -- prepare to 4 char dec
              device:send(cluster_base.write_manufacturer_specific_attribute(device, info.clusterId, info.attributeId, common.MFG_CODE, info.dataType, offset))
            -- enable window open state detection feature
            elseif (name == common.WINDOW_OPEN_FEATURE) then 
              device:send(cluster_base.write_manufacturer_specific_attribute(device, info.clusterId, info.attributeId, common.MFG_CODE, info.dataType, input))
            end
        end
    end
...
local popp_thermostat = {
  ...
  capability_handlers = {
    ...
    [capabilities.thermostatHeatingSetpoint.ID] = {
      [capabilities.thermostatHeatingSetpoint.commands.setHeatingSetpoint.NAME] = set_setpoint_factory(Thermostat.attributes.OccupiedHeatingSetpoint)
    },
    [HeatingMode.ID] = {
