...
common.window_open_detection_handler = function(driver, device, value, zb_rx)
    --log.debug("### Window value.value:" .. tostring(value.value))
    --log.debug("### common.WINDOW_OPEN_FEATURE : " .. tostring(common.WINDOW_OPEN_FEATURE)) 
...
common.heat_cmd_handler = function(driver, device, command)
    ...
    if last_setpointTemp == nil then
      last_setpointTemp = device:get_latest_state("main", capabilities.thermostatHeatingSetpoint.ID, capabilities.thermostatHeatingSetpoint.heatingSetpoint.NAME)
    end
