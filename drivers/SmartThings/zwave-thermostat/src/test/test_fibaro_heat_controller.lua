...
local mock_device_extended = test.mock_device.build_test_zwave_device(
  {
    profile = t_utils.get_profile_definition("fibaro-heat-extra-sensor.yml"),
    zwave_endpoints = thermostat_endpoints,
    zwave_manufacturer_id = 0x010F,
    zwave_product_id = 0x1000,
    zwave_product_type = 0x1301,
...
test.register_message_test(
    "Refresh Capability Command should refresh Thermostat device with extra temperature sensor",
    {
      ...
      },
      {
        ...
        message = zw_test_utilities.zwave_test_build_send_command(
          mock_device_extended,
          SensorMultilevel:Get({}, {encap = zw.ENCAP.AUTO, src_channel = 0, dst_channels = {2}})
        )
      },
      {
        channel = "zwave",
...
test.register_coroutine_test(
    ...
    function()
      ...
      test.socket.zwave:__queue_receive({
        mock_device.id,
        ApplicationStatus:ApplicationBusy({ wait_time = 2 })
      })
