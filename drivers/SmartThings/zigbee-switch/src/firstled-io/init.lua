...
--When stse.changeToWirelessSwitch switching to the wirelessSwitch mode to listen for profile changes
local function listen_profile_button_transition(device, args)
  local current_has = device:supports_capability(capabilities.button, "main")

  local old_has = false
  if args and args.old_st_store and args.old_st_store.profile then
    local old_main = args.old_st_store.profile.components.main
    if old_main and old_main.capabilities then
      old_has = old_main.capabilities["button"] ~= nil
    end
  end
  --Capabilities button from non-existent to existing
  if not old_has and current_has then
...
local function device_info_changed(driver, device, event, args)
  listen_profile_button_transition(device, args)
