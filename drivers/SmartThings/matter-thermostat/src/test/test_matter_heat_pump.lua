...
local mock_device_with_auto = test.mock_device.build_test_matter_device({
  ...
  endpoints = {
    ...
    },
    {
      endpoint_id = THERMOSTAT_ONE_EP,
      clusters = {
        { cluster_id = clusters.Thermostat.ID, cluster_type = "SERVER", feature_map = 35  }, --AUTO, HEAT & COOL
        { cluster_id = clusters.TemperatureMeasurement.ID, cluster_type = "SERVER" },
        { cluster_id = clusters.RelativeHumidityMeasurement.ID, cluster_type = "SERVER" },
      },
      device_types = {
...
local test_init_common = function(device)
  ...
  test.socket.matter:__expect_send({
    device.id, clusters.ElectricalEnergyMeasurement.attributes.CumulativeEnergyImported:read(device, HEAT_PUMP_EP)
  })
...
test.register_message_test(
  "Thermostat mode reports from the component endpoints should generate correct messages to the right component",
  {
    ...
    },
    {
      ...
      channel = "capability",
      direction = "send",
      message = mock_device:generate_test_message("thermostatOne", capabilities.thermostatMode.supportedThermostatModes({"emergency heat"}, {visibility={displayed=false}}))
    },
    {
      channel = "matter",
      direction = "receive",
...
test.register_message_test(
  "Additional mode reports from component thermostat endpoints should extend the supported modes for their corresponding components when auto is supported",
  {
    ...
    },
    {
      channel = "capability",
      direction = "send",
      message = mock_device_with_auto:generate_test_message("thermostatOne", capabilities.thermostatMode.supportedThermostatModes({"off", "auto", "cool", "heat"}, {visibility={displayed=false}}))
    },
		{
      channel = "matter",
      direction = "receive",
      ...
      channel = "capability",
      direction = "send",
      message = mock_device_with_auto:generate_test_message("thermostatOne", capabilities.thermostatMode.supportedThermostatModes({"off", "auto", "cool", "heat", "emergency heat"}, {visibility={displayed=false}}))
    },
    {
      channel = "matter",
      direction = "receive",
    ...
    },
    {
      channel = "capability",
      direction = "send",
      message = mock_device_with_auto:generate_test_message("thermostatTwo", capabilities.thermostatMode.supportedThermostatModes({"off", "auto", "cool", "heat"}, {visibility={displayed=false}}))
    },
		{
      channel = "matter",
      direction = "receive",
      ...
      channel = "capability",
      direction = "send",
      message = mock_device_with_auto:generate_test_message("thermostatTwo", capabilities.thermostatMode.supportedThermostatModes({"off", "auto", "cool", "heat", "emergency heat"}, {visibility={displayed=false}}))
    }
  },
...
test.register_coroutine_test(
  ...
  function()
    ...
    test.socket.matter:__expect_send({
      mock_device.id, clusters.ElectricalEnergyMeasurement.attributes.CumulativeEnergyImported:read(mock_device, HEAT_PUMP_EP)
    })
