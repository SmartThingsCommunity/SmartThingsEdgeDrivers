...
local PeriodicEnergyMeasured = {
  ID = 0x0001,
  NAME = "PeriodicEnergyMeasured",
  base_type = data_types.Structure,
}

PeriodicEnergyMeasured.field_defs = {
  {
...
function PeriodicEnergyMeasured:augment_type(base_type_obj)
  local elems = {}
...
function PeriodicEnergyMeasured:read(device, endpoint_id)
...
function PeriodicEnergyMeasured:subscribe(device, endpoint_id)
...
function PeriodicEnergyMeasured:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "PeriodicEnergyMeasuredEventData", ID = 0x15}))
...
function PeriodicEnergyMeasured:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function PeriodicEnergyMeasured:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

return PeriodicEnergyMeasured
