...
test.register_coroutine_test(
  ...
  function()
    ...
    test.socket.zigbee:__expect_send({ mock_device.id, PowerConfiguration.attributes.BatteryVoltage:read(mock_device) })
    test.socket.zigbee:__expect_send({ mock_device.id, TemperatureMeasurement.attributes.MeasuredValue:read(mock_device) })
    test.socket.zigbee:__expect_send({ mock_device.id, PollControl.attributes.CheckInInterval:write(mock_device, 0x00001C20) })
    test.socket.zigbee:__expect_send({ mock_device.id, PollControl.commands.SetShortPollInterval(mock_device, 0x0200) })
    test.socket.zigbee:__expect_send({ mock_device.id, PollControl.attributes.FastPollTimeout:write(mock_device, 0x0028) })
    test.socket.zigbee:__expect_send({ mock_device.id, PollControl.commands.SetLongPollInterval(mock_device, 0xB1040000) })
