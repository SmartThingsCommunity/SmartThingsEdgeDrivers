...
local mock_device = test.mock_device.build_test_zigbee_device(
  {
    ...
    data = {
      firmwareFullVersion = "1f106532"
    }
  }
)

local mock_device_old_firmware = test.mock_device.build_test_zigbee_device(
  {
    profile = t_utils.get_profile_definition("multi-sensor.yml"),
    zigbee_endpoints = {
      [1] = {
...
test.register_coroutine_test(
  ...
  function()
    ...
    test.wait_for_events()
    local attr_report_data = {
      { 0x0013, data_types.Int16.ID, 901}
    }
    test.socket.zigbee:__queue_receive({
      mock_device.id,
      zigbee_test_utils.build_attribute_report(mock_device, 0xFC02, attr_report_data, 0x110A)
...
test.register_coroutine_test(
  ...
  function()
    ...
    test.socket.capability:__expect_send(mock_device:generate_test_message("main", capabilities.accelerationSensor.acceleration.inactive()))
    test.socket.capability:__expect_send(mock_device:generate_test_message("main", capabilities.threeAxis.threeAxis({value={0,0,0}})))
    test.socket.zigbee:__expect_send({
      mock_device.id,
      PowerConfiguration.attributes.BatteryVoltage:read(mock_device)
    })
...
test.register_coroutine_test(
  ...
  function()
    ...
    test.socket.zigbee:__expect_send({
      mock_device.id,
      zigbee_test_utils.build_attr_config(mock_device, 0xFC02, 0x0010, 10, 3600, data_types.Bitmap8, 1, 0x104E)
    })
    test.socket.zigbee:__expect_send({
      mock_device.id,
      zigbee_test_utils.build_attr_config(mock_device, 0xFC02, 0x0012, 1, 3600, data_types.Int16, 1, 0x104E)
    })
    test.socket.zigbee:__expect_send({
      mock_device.id,
      zigbee_test_utils.build_attr_config(mock_device, 0xFC02, 0x0013, 1, 3600, data_types.Int16, 1, 0x104E)
    })
    test.socket.zigbee:__expect_send({
      mock_device.id,
      zigbee_test_utils.build_attr_config(mock_device, 0xFC02, 0x0014, 1, 3600, data_types.Int16, 1, 0x104E)
    })
...
test.register_coroutine_test(
  ...
  function()
    ...
    for voltage, batt_perc in pairs(battery_test_map[mock_device_old_firmware:get_manufacturer()]) do
      test.socket.zigbee:__queue_receive({ mock_device_old_firmware.id, PowerConfiguration.attributes.BatteryVoltage:build_test_attr_report(mock_device_old_firmware, voltage) })
