...
command_mt.__index = function(self, key)
  if command_mt.__command_cache[key] == nil then
    local req_loc = string.format("RvcRunMode.server.commands.%s", key)
...
local RvcRunModeServerCommands = {}

function RvcRunModeServerCommands:set_parent_cluster(cluster)
...
local status, aliases = pcall(require, "st.matter.clusters.aliases.RvcRunMode.server.commands")
if status then
  aliases:add_to_class(RvcRunModeServerCommands)
end
