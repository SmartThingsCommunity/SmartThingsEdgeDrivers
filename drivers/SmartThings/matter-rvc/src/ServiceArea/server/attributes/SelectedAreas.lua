...
--- @field public NAME string "SelectedAreas" the name of this attribute
...
function SelectedAreas:augment_type(data_type_obj)
  for i, v in ipairs(data_type_obj.elements) do
    data_type_obj.elements[i] = data_types.validate_or_build_type(v, SelectedAreas.element_type)
  end
end
...
function SelectedAreas:new_value(...)
...
function SelectedAreas:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return any
function SelectedAreas:subscribe(device, endpoint_id)
...
function SelectedAreas:set_parent_cluster(cluster)
...
function SelectedAreas:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function SelectedAreas:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)

  return data
end

setmetatable(SelectedAreas, {__call = SelectedAreas.new_value, __index = SelectedAreas.base_type})
