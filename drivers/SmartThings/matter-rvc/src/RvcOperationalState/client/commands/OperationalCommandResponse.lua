...
local OperationalCommandResponse = {}

OperationalCommandResponse.NAME = "OperationalCommandResponse"
OperationalCommandResponse.ID = 0x0004
OperationalCommandResponse.field_defs = {
  {
    name = "command_response_state",
    field_id = 0,
    is_nullable = false,
    is_optional = false,
...
function OperationalCommandResponse:augment_type(base_type_obj)
  local elems = {}
...
function OperationalCommandResponse:build_test_command_response(device, endpoint_id, command_response_state, interaction_status)
  local function init(self, device, endpoint_id, command_response_state)
    local out = {}
    local args = {command_response_state}
    ...
    setmetatable(out, {
      __index = OperationalCommandResponse,
      __tostring = OperationalCommandResponse.pretty_print
    })
  ...
  local self_request =  init(self, device, endpoint_id, command_response_state)
...
function OperationalCommandResponse:init()
  return nil
end

function OperationalCommandResponse:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

function OperationalCommandResponse:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(OperationalCommandResponse, {__call = OperationalCommandResponse.init})
