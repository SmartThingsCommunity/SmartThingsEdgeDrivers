...
-- Helper functions --
function update_supported_mode(device, current_run_mode, operating_state)
  device.log.info_with({hub_logs = true}, string.format("!!!! update_supported_mode: %s, %s", current_run_mode, operating_state))
  ...
  local supported_run_modes = device:get_field(RUN_MODE_SUPPORTED_MODES)
  ...
  device.log.info_with({hub_logs = true}, string.format("!!!! current_tag: %d", current_tag))
  ...
  device.log.info_with({hub_logs = true}, string.format("!!!! can_be_selected_as_non_idle: %s", can_be_selected_as_non_idle))

  -- Set supported run modes and arguments
  local supported_modes = {} -- For automation
  local supported_arguments = {} -- For generic plugin
  for i, mode in ipairs(supported_run_modes) do
    table.insert(supported_modes, mode[1])
  ...
  -- Send event to set supported run modes
  local component = device.profile.components["runMode"]
  local event = capabilities.mode.supportedModes(supported_modes, {visibility = {displayed = false}})
  device:emit_component_event(component, event)
  event = capabilities.mode.supportedArguments(supported_arguments, {visibility = {displayed = false}})
  device:emit_component_event(component, event)

  -- Set supported clean modes and arguments
  local supported_clean_modes = device:get_field(CLEAN_MODE_SUPPORTED_MODES)
  supported_modes = {}
...
local function run_mode_supported_mode_handler(driver, device, ib, response)
  ...
  for i, mode in ipairs(ib.data.elements) do
    ...
    for i, t in ipairs(mode.elements.mode_tags.elements) do
      device.log.info_with({hub_logs = true}, string.format("!!!! tag: %s", t.elements.value.value))
    ...
    table.insert(supported_modes, {mode.elements.label.value, tag})
    device.log.info_with({hub_logs = true}, string.format("!!!! run_mode_supported_mode_handler: %s, %s", mode.elements.label.value, tag))
  end
  device:set_field(RUN_MODE_SUPPORTED_MODES, supported_modes, { persist = true })
  ...
  update_supported_mode(device, current_run_mode, operating_state)
end

local function run_mode_current_mode_handler(driver, device, ib, response)
  device.log.info_with({ hub_logs = true }, string.format("run_mode_current_mode_handler currentMode: %s", ib.data.value))
...
local function clean_mode_supported_mode_handler(driver, device, ib, response)
  device.log.info_with({ hub_logs = true }, string.format("clean_mode_supported_mode_handler"))
  ...
  for i, mode in ipairs(ib.data.elements) do
    ...
    device.log.info_with({hub_logs = true}, string.format("!!!! run_mode_supported_mode_handler: %s", mode.elements.label.value))
  end
...
local function clean_mode_current_mode_handler(driver, device, ib, response)
  device.log.info_with({ hub_logs = true }, string.format("clean_mode_current_mode_handler currentMode: %s", ib.data.value))
...
local function rvc_operational_state_attr_handler(driver, device, ib, response)
  device.log.info_with({ hub_logs = true }, string.format("rvc_operational_state_attr_handler operationalState: %s", ib.data.value))
  ...
  device.log.info_with({ hub_logs = true }, string.format("!!!! rvc_operational_state_attr_handler %s, %s", ib.data.value, clus_op_enum.ERROR))
  ...
  if ib.data.value ~= clus_op_enum.ERROR then
    update_supported_mode(device, current_run_mode, OPERATING_STATE_MAP[ib.data.value].NAME)
  else
    update_supported_mode(device, current_run_mode, "Error")
  end
end
...
local function rvc_operational_error_attr_handler(driver, device, ib, response)
  ...
  device.log.info_with({ hub_logs = true }, string.format("rvc_operational_error_attr_handler errorStateID: %s", ib.data.elements.error_state_id.value))
...
local function handle_robot_cleaner_mode(driver, device, cmd)
  device.log.info_with({ hub_logs = true }, string.format("handle_robot_cleaner_mode component: %s, mode: %s", cmd.component, cmd.args.mode))
