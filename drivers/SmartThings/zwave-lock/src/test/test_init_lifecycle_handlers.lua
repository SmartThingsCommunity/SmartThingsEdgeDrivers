...
do
  ...
  test.register_coroutine_test(
    ...
    function()
      ...
      -- Refresh sends: DoorLock:OperationGet, Battery:Get, UserCode:UsersNumberGet
      test.socket.zwave:__expect_send(DoorLock:OperationGet({}):build_test_tx(dev.id))
      test.socket.zwave:__expect_send(Battery:Get({}):build_test_tx(dev.id))
      ...
      -- Delayed sync_device_state (2 s) sends UserCode:Get(1)
      test.mock_time.advance_time(2)
      test.socket.zwave:__expect_send(
        UserCode:Get({ user_identifier = 1 }):build_test_tx(dev.id))
      test.wait_for_events()
    end,
    {
      test_init = function()
        test.disable_startup_messages()
        dev = test.mock_device.build_test_zwave_device({
          profile         = t_utils.get_profile_definition("lock-battery.yml"),
          zwave_endpoints = zwave_lock_endpoints,
        })
...
do
  ...
  test.register_coroutine_test(
    ...
    function()
      ...
      test.wait_for_events()
      -- No capability events, no z-wave sends expected
    end,
    {
      test_init = function()
        test.disable_startup_messages()
...
do
  ...
  test.register_coroutine_test(
    ...
    function()
      ...
      test.wait_for_events()
      -- profile_switched is true but new profile has no lockCodes → no migration events
    end,
    {
      test_init = function()
        test.disable_startup_messages()
