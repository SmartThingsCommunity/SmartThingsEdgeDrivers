...
test.register_message_test(
  "Lock notification reporting should be handled",
  {
    ...
    -- not a special case for this lock, should be handled as usual
    {
      ...
      message = {
        mock_device.id,
        Notification:Report({notification_type = 6, event = 6, event_parameter = "\x01"})
      }
    },
    {
      channel = "capability",
      direction = "send",
      message = mock_device:generate_test_message("main", capabilities.lock.lock.unlocked({data={method="keypad"}}))
    }
  }
)
