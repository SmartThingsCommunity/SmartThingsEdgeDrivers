...
function EVConnected:augment_type(base_type_obj)
  local elems = {}
...
function EVConnected:read(device, endpoint_id)
...
function EVConnected:subscribe(device, endpoint_id)
...
function EVConnected:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "EVConnectedEventData", ID = 0x15}))
...
function EVConnected:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function EVConnected:set_parent_cluster(cluster)
