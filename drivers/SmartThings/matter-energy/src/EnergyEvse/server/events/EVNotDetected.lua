...
EVNotDetected.field_defs = {
  ...
  },
  {
    ...
    field_id = 4,
    is_array = false,
    name = "session_energy_discharged",
    is_nullable = false,
    is_optional = false,
  },
}

function EVNotDetected:augment_type(base_type_obj)
  local elems = {}
...
function EVNotDetected:read(device, endpoint_id)
...
function EVNotDetected:subscribe(device, endpoint_id)
...
function EVNotDetected:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "EVNotDetectedEventData", ID = 0x15}))
...
function EVNotDetected:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function EVNotDetected:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

return EVNotDetected
