...
local SubscriptionMap = {
  ...
  conditional_subscriptions = {
    [function(device) return #device:get_endpoints(clusters.CameraAvStreamManagement.ID) > 0 end] = {
      clusters.CameraAvStreamManagement.attributes.AttributeList,
      camera_fields.CameraAVSMFeatureMapAttr
    },
    [function(device) return #device:get_endpoints(clusters.CameraAvSettingsUserLevelManagement.ID) > 0 end] = {
      camera_fields.CameraAVSULMFeatureMapAttr
    },
    [function(device) return #device:get_endpoints(clusters.ZoneManagement.ID) > 0 end] = {
      camera_fields.ZoneManagementFeatureMapAttr,
    }
  }
}
