...
function CameraDeviceConfiguration.assign_profile_for_camera_ep(device, camera_ep_id)
  ...
  if camera_ep_id then
    local camera_ep = switch_utils.get_endpoint_info(device, camera_ep_id)
  ...
  if device:get_field(fields.profiling_data.STATUS_LIGHT_ENABLED_PRESENT) then
    table.insert(status_led_component_capabilities, capabilities.switch.ID)
  end
  if device:get_field(fields.profiling_data.STATUS_LIGHT_BRIGHTNESS_PRESENT) then
  ...
  if camera_utils.optional_capabilities_list_changed(optional_supported_component_capabilities, device.profile.components) then
    CameraDeviceConfiguration.update_camera_component_map(device)
    if #doorbell_endpoints > 0 then
      CameraDeviceConfiguration.update_doorbell_component_map(device, doorbell_endpoints[1])
    end
    return "camera", optional_supported_component_capabilities
  end
end
...
local function initialize_selected_camera_capabilities(device, capabilities_to_reinit)
  ...
  if reinit_targets.mechanicalPanTiltZoom then
    init_ptz(device)
  end
  if reinit_targets.zoneManagement then
    init_zone_management(device)
  end
  if reinit_targets.localMediaStorage then
    init_local_media_storage(device)
  end
  if reinit_targets.audioRecording then
    init_audio_recording(device)
  end
  if reinit_targets.videoStreamSettings then
    init_video_stream_settings(device)
  end
  if reinit_targets.cameraPrivacyMode then
...
local function changed_capabilities_from_profiles(old_profile, new_profile)
  ...
  for _, capability_id in ipairs(managed_capabilities) do
    if old_set[capability_id] ~= new_set[capability_id] and new_set[capability_id] == true then
      flags[capability_id] = true
    end
  end
...
function CameraDeviceConfiguration.reconcile_profile_and_capabilities(driver, device)
  local device_cfg = require "switch_utils.device_configuration".DeviceCfg
  local profile_update_requested = device_cfg.match_profile(driver, device)
...
function CameraDeviceConfiguration.reinitialize_changed_camera_capabilities(device, old_profile, new_profile)
...
function CameraDeviceConfiguration.update_camera_component_map(device)
  ...
  if #camera_av_ep_ids > 0 then
    ...
    local component_map = device:get_field(fields.COMPONENT_TO_ENDPOINT_MAP) or {}
    if camera_utils.feature_supported(device, clusters.CameraAvStreamManagement.ID, clusters.CameraAvStreamManagement.types.Feature.AUDIO) then
