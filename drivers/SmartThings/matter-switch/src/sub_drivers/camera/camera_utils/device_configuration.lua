...
function CameraDeviceConfiguration.update_camera_component_map(device)
  ...
  local video_camera_av_ep_ids = device:get_endpoints(clusters.CameraAvStreamManagement.ID, {feature_bitmap=clusters.CameraAvStreamManagement.types.Feature.VIDEO})
  if #video_camera_av_ep_ids > 0 then
    camera_utils.update_component_to_endpoint_map(device, camera_fields.profile_components.speaker, {
      endpoint_id = video_camera_av_ep_ids[1],
      cluster_id = clusters.CameraAvStreamManagement.ID,
      attribute_ids = {
