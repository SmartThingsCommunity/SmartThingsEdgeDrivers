...
local function get_matter_device(device)
    ...
    if matter_device_id then
        ...
        if driver then
            ...
            if matter_device then return matter_device end
        end
    end
    log.warn("Could not find child device, returning current device", device.id)
    return nil
end
...
local function find_child(parent_device, endpoint_id)
    if type(endpoint_id) == "number" then
        local child = parent_device:get_child_by_parent_assigned_key(string.format("%d", endpoint_id ))
        if child then
            return child
        end
    end
    local host = get_matter_device(parent_device)
    return host
end
...
local function device_init(driver, device)
    ...
    if matter_device and not main_onOff_at_join and (product_id == 0x0005 or product_id == 0x0006) then
        matter_device:set_field(MAIN_ONOFF_EP, 3, { persist = true })
        device.thread:call_with_delay(6, function()
            if matter_device:supports_capability(capabilities.switchLevel) then
                matter_device:set_field(MAIN_ONOFF_EP, 4, { persist = true })
            end
        end)
    end
    ...
    for _, ep in ipairs(removed) do
        ...
        elseif ep == 4 then
            if device.manufacturer_info.product_id == 0x0006 then
                ...
                if  button_comb then
                    matter_device:try_update_metadata({ profile = "2-button" })
                    create_child(driver, parent, {3}, 1,create_assign_profile_wrapper(100))
                else
...
local function handle_set_preset(driver, device, cmd)
    local endpoint_id = tonumber(device:get_field(MAIN_WC_EP)) or device:component_to_endpoint(cmd.component)
...
local function handle_close(driver, device, cmd)
    local endpoint_id = device:get_field(MAIN_WC_EP) or device:component_to_endpoint(cmd.component)
...
local function info_changed(driver, device, event, args)
    ...
    if device.network_type == device_lib.NETWORK_TYPE_MATTER and device.profile.id ~= args.old_st_store.profile.id then
        matter_device:set_endpoint_to_component_fn(switch_utils.endpoint_to_component)
        matter_device.thread:call_with_delay(5, function()
            if matter_device:supports_capability(capabilities.button) then
                ...
                for _, ep in ipairs(clean_eps) do
                    ...
                    subscribe(parent, ep, clusters.Switch.ID, nil, clusters.Switch.events.ShortRelease.ID)
                    subscribe(parent, ep, clusters.Switch.ID, nil, clusters.Switch.events.LongPress.ID)
                    matter_device:emit_event_for_endpoint(ep, capabilities.button.supportedButtonValues({ "pushed", "double", "held" }))
                end
            elseif matter_device:supports_capability(capabilities.switch) then
                parent:send(clusters.OnOff.attributes.OnOff:read(parent))
            elseif matter_device:supports_capability(capabilities.motionSensor) then
            ...
            elseif matter_device:supports_capability(capabilities.windowShadeLevel) then
...
local function device_type_handler (driver, device, ib)
    ...
    for _, element in ipairs(value) do
        ...
        if device_type_id == 15 then
            if not switch_utils.tbl_contains(button_endpoints, ep) then
                switch_utils.set_field_for_endpoint(parent, SUPPORTS_MULTI_PRESS, ep, true, {persist = true})
                table.insert(button_endpoints, ep)
                table.sort(button_endpoints)
                parent:set_field(BUTTON_EPS, button_endpoints, { persist = true })
            end
        end
        ...
        if device_type_id == fields.DEVICE_TYPE_ID.LIGHT.ON_OFF then
            ...
            elseif ep == 3 and device.manufacturer_info.product_id == 0x0006 then
                matter_device.thread:call_with_delay(4, function()
                    local latest_eps = matter_device:get_field(ACTIVE_EPS) or {}
                    if not switch_utils.tbl_contains(latest_eps, 4) then
