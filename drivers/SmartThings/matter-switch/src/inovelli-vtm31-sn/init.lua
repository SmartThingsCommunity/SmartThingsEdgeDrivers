...
local INOVELLI_VTM31_SN_FINGERPRINTS = {
  { vendor_id = 0x1361, product_id = 0x0001 },
}
...
local is_inovelli_vtm31_sn = function(opts, driver, device)
  for _, fingerprint in ipairs(INOVELLI_VTM31_SN_FINGERPRINTS) do
      ...
      for id, value in pairs(device.preferences) do
        if args.old_st_store.preferences[id] ~= value and preferences and preferences[id] then
          ...
          if(preferences[id].size == data_types.Boolean) then
            device:send(cluster_base.write(device, 0x01, PRIVATE_CLUSTER_ID, preferences[id].parameter_number, nil, to_boolean(new_parameter_value)))
          else
            device:send(cluster_base.write(device, 0x01, PRIVATE_CLUSTER_ID, preferences[id].parameter_number, nil, new_parameter_value))
          end
        end
      end
      --device:send(cluster_base.read(device, 0x01, data_types.ClusterId(0x0000), 0x4000, nil))
    end
  end
end
...
local do_configure = function(self, device)
  if device.network_type ~= device_lib.NETWORK_TYPE_CHILD then
    ...
    device:send(cluster_base.read(device, 0x01, PRIVATE_CLUSTER_ID, 21, nil))
    --device:send(cluster_base.read(device, 0x01, data_types.ClusterId(0x0000), 0x4000, nil))

    ---- Additional one time configuration
    --if  device:supports_capability(capabilities.powerMeter) then
    --  -- Divisor and multiplier for PowerMeter
    ...
    --  -- Divisor and multiplier for EnergyMeter
...
local function on_handler(driver, device, cmd)
  if device.network_type ~= device_lib.NETWORK_TYPE_CHILD then
    ...
    local send_configuration = function()
      parent_device:send(cluster_base.build_cluster_command(
        --self,
        parent_device,
...
local function off_handler(driver, device, cmd)
  if device.network_type ~= device_lib.NETWORK_TYPE_CHILD then
    local endpoint_id = device:component_to_endpoint(cmd.component)
...
local function switch_level_handler(driver, device, cmd)
  if device.network_type ~= device_lib.NETWORK_TYPE_CHILD then
    log.info(string.format("Set Level, Component: %s", cmd.component))
    log.info(string.format("Set Level, EP: %d", device:component_to_endpoint(cmd.component)))
    local endpoint_id = device:component_to_endpoint(cmd.component)
    ...
    device:emit_event(capabilities.switch.switch(cmd.args.level ~= 0 and "on" or "off"))
    local parent_device = device:get_parent_device()
    local send_configuration = function()
      parent_device:send(cluster_base.build_cluster_command(
        --self,
...
local function set_color_temperature(driver, device, cmd)
  ...
  device:emit_event(capabilities.colorTemperature.colorTemperature(cmd.args.temperature))
  local parent_device = device:get_parent_device()
  local send_configuration = function()
    parent_device:send(cluster_base.build_cluster_command(
      --self,
