...
ValveFault.field_defs = {
  {
    name = "valve_fault",
    field_id = 0,
    is_nullable = false,
    is_optional = false,
    data_type = require "st.matter.generated.zap_clusters.ValveConfigurationAndControl.types.ValveFaultBitmap",
  },
}

function ValveFault:augment_type(base_type_obj)
  local elems = {}
...
function ValveFault:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "ValveFaultEventData", ID = 0x15}))
