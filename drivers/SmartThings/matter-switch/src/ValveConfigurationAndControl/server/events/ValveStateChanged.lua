...
local ValveStateChanged = {
  ID = 0x0000,
  NAME = "ValveStateChanged",
  base_type = data_types.Structure,
}

ValveStateChanged.field_defs = {
  {
    name = "valve_state",
    field_id = 0,
    is_nullable = false,
    is_optional = false,
    data_type = require "st.matter.generated.zap_clusters.ValveConfigurationAndControl.types.ValveStateEnum",
  },
  {
    name = "valve_level",
    field_id = 1,
...
function ValveStateChanged:augment_type(base_type_obj)
  local elems = {}
...
function ValveStateChanged:read(device, endpoint_id)
...
function ValveStateChanged:subscribe(device, endpoint_id)
...
function ValveStateChanged:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "ValveStateChangedEventData", ID = 0x15}))
...
function ValveStateChanged:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function ValveStateChanged:set_parent_cluster(cluster)
