...
function Close:build_test_command_response(device, endpoint_id, status)
...
function Close:init(device, endpoint_id)
  local out = {}
  local args = {}
...
function Close:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

function Close:augment_type(base_type_obj)
  local elems = {}
...
function Close:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(Close, {__call = Close.init})
