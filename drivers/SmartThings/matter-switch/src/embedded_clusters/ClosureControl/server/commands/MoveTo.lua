...
function MoveTo:build_test_command_response(device, endpoint_id, status)
...
function MoveTo:init(device, endpoint_id, position, latch, speed)
  local out = {}
  local args = {position, latch, speed}
  ...
  setmetatable(out, {
    __index = MoveTo,
    __tostring = MoveTo.pretty_print
  })
...
function MoveTo:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

function MoveTo:augment_type(base_type_obj)
  local elems = {}
...
function MoveTo:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(MoveTo, {__call = MoveTo.init})
