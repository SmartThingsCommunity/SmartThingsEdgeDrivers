...
local function send_import_poll_report(device, latest_total_imported_energy_wh)
  ...
  device:emit_component_event("main",capabilities.powerConsumptionReport.powerConsumption({
...
local function set_poll_report_timer_and_schedule(device, is_cumulative_report)
  ...
  elseif not device:get_field(FIRST_IMPORT_REPORT_TIMESTAMP) then
    ...
    if device:get_field(ELECTRICAL_SENSOR_EPS) ~= {} then
  ...
  for _, ep in ipairs(switch_eps) do
    if device:supports_server_cluster(clusters.OnOff.ID, ep) then
      ...
      if ep ~= main_endpoint then -- don't create a child device that maps to the main endpoint
        ...
        local electrical_topology = device:get_field(profiling_data.ELECTRICAL_TOPOLOGY)
        if electrical_topology.topology == clusters.PowerTopology.types.Feature.SET_TOPOLOGY then
            electrical_tags = electrical_topology.tags_on_set_ep[ep] or ""
        end
...
local function collect_and_store_electrical_sensor_info(driver, device)
  ...
  for _, ep in (device.endpoints) do
    for _, dt in (ep.device_types) do
      if dt.device_type_id == ELECTRICAL_SENSOR_ID then
        local electrical_ep_info = {}
        for _, cluster in (ep.clusters) do
        ...
        electrical_sensor_eps[ep.endpoint_id] = electrical_ep_info
        break
      end
    end
  end
  ...
  if device.network_type == device_lib.NETWORK_TYPE_MATTER then
    ...
    for _, ep in ipairs(device.endpoints) do
      if ep.endpoint_id ~= main_endpoint then
        ...
        for _, attr in pairs(device_type_attribute_map[ELECTRICAL_SENSOR_ID] or {}) do
...
local function match_profile(driver, device)
  ...
  local electrical_tags = ""
  local topology = device:get_field(profiling_data.ELECTRICAL_TOPOLOGY).topology
  if topology == clusters.PowerTopology.types.Feature.NODE_TOPOLOGY then
    -- Will only loop through one endpoint by definition of NODE_TOPOLOGY feature
    for ep_info in pairs(device:get_field(ELECTRICAL_SENSOR_EPS)) do
        if ep_info.power then electrical_tags = electrical_tags + "-power" end
        if ep_info.energy then electrical_tags = electrical_tags + "-energy-powerConsumption" end
    end
  end
...
local function available_endpoints_handler(driver, device, ib, response)
  local available_eps = {}
  for _, ep in pairs(ib.data.elements) do
    table.insert(available_eps, ep)
  end
  available_eps.sort()
  local electrical_sensor_eps = device:get_field(ELECTRICAL_SENSOR_EPS)
  electrical_sensor_eps[ib.endpoint_id].availableEndpoints = available_eps
  device:set_field(ELECTRICAL_SENSOR_EPS, electrical_sensor_eps)
  local avail_eps_req_complete = true
  local electrical_tags_per_ep = {}
  for ep_info in pairs(electrical_sensor_eps) do
    if ep_info.availableEndpoints == false then
      ...
      if ep_info.power then electrical_tags = electrical_tags + "-power" end
      if ep_info.energy then electrical_tags = electrical_tags + "-energy-powerConsumption" end
      electrical_tags_per_ep[ep_info.availableEndpoints[1]] = electrical_tags
    end
  end
  if avail_eps_req_complete then
    local topology_data = {}
    topology_data.topology = clusters.PowerTopology.types.Feature.SET_TOPOLOGY
    topology_data.tags_on_set_ep = electrical_tags_per_ep
...
local function cumul_energy_imported_handler(driver, device, ib, response)
  if ib.data.elements.energy then
    ...
    device:emit_component_event("main", capabilities.energyMeter.energy({ value = watt_hour_value, unit = "Wh" }))
  end
end
...
local function active_power_handler(driver, device, ib, response)
  if ib.data.value then
    ...
    device:emit_component_event("main", capabilities.powerMeter.power({ value = watt_value, unit = "W"}))
  end
end
...
local function device_added(driver, device)
  ...
  elseif device.network_type == device_lib.NETWORK_TYPE_MATTER then
    ...
    local electrical_sensor_eps = device:get_field(ELECTRICAL_SENSOR_EPS)
    local node_topology = clusters.PowerTopology.types.Feature.NODE_TOPOLOGY
    local min_electrical_ep = math.huge
    for ep in pairs(electrical_sensor_eps) do
        min_electrical_ep = math.min(ep, min_electrical_ep)
    end
    if electrical_sensor_eps == {} then
      local topology_data = {}
      topology_data.topology = false
      device:set_field(profiling_data.ELECTRICAL_TOPOLOGY, topology_data)
    elseif electrical_sensor_eps[min_electrical_ep].topology ~= node_topology then
      local topology_data = {}
      topology_data.topology = clusters.PowerTopology.types.Feature.NODE_TOPOLOGY
      topology_data.node_ep = min_electrical_ep
      device:set_field(profiling_data.ELECTRICAL_TOPOLOGY, topology_data)
    end
  end
