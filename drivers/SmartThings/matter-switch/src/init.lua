...
local function handle_set_color_temperature(driver, device, cmd)
  ...
  local min_temp_kelvin = get_field_for_endpoint(device, COLOR_TEMP_BOUND_RECEIVED..COLOR_TEMP_MIN, endpoint_id)
  local max_temp_kelvin = get_field_for_endpoint(device, COLOR_TEMP_BOUND_RECEIVED..COLOR_TEMP_MAX, endpoint_id)
...
local function temp_attr_handler(driver, device, ib, response)
  ...
  if min_temp_mired ~= nil and temp_in_mired <= min_temp_mired then
    temp = get_field_for_endpoint(device, COLOR_TEMP_BOUND_RECEIVED..COLOR_TEMP_MAX, ib.endpoint_id)
  elseif max_temp_mired ~= nil and temp_in_mired >= max_temp_mired then
    temp = get_field_for_endpoint(device, COLOR_TEMP_BOUND_RECEIVED..COLOR_TEMP_MIN, ib.endpoint_id)
  end
...
local mired_bounds_handler_factory = function(minOrMax)
  return function(driver, device, ib, response)
    ...
    set_field_for_endpoint(device, COLOR_TEMP_BOUND_RECEIVED..minOrMax, ib.endpoint_id, temp_in_kelvin, {persist = true})
    -- the minimum color temp in kelvin corresponds to the maximum temp in mireds
    if minOrMax == COLOR_TEMP_MIN then
      set_field_for_endpoint(device, COLOR_TEMP_BOUND_RECEIVED_MIRED..COLOR_TEMP_MAX, ib.endpoint_id, temp_in_mired, {persist = true})
    else
      set_field_for_endpoint(device, COLOR_TEMP_BOUND_RECEIVED_MIRED..COLOR_TEMP_MIN, ib.endpoint_id, temp_in_mired, {persist = true})
    end
