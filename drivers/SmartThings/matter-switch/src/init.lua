  ...
  if not switch_utils.deep_equals(device.profile, args.old_st_store.profile, { ignore_functions = true }) then
    if device.network_type == device_lib.NETWORK_TYPE_MATTER then
      ...
      if version.api >= 16 and version.rpc >= 10 then
        ...
        camera_cfg.reinitialize_changed_camera_capabilities(device, args.old_st_store.profile, device.profile)
      end
    elseif device.network_type == device_lib.NETWORK_TYPE_CHILD then
...
function SwitchLifecycleHandlers.device_init(driver, device)
  if device.network_type == device_lib.NETWORK_TYPE_MATTER then
    ...
    if version.api < 16 or version.rpc < 10 or #device:get_endpoints(clusters.CameraAvStreamManagement.ID) == 0 then
      device:set_field(fields.profiling_data.STATUS_LIGHT_ENABLED_PRESENT, false, {persist = true})
      device:set_field(fields.profiling_data.STATUS_LIGHT_BRIGHTNESS_PRESENT, false, {persist = true})
    end
