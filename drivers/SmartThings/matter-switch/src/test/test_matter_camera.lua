...
local CAMERA_EP, FLOODLIGHT_EP, MOTION_EP = 1, 2, 3

local mock_device = test.mock_device.build_test_matter_device({
  profile = t_utils.get_profile_definition("matter-camera.yml"),
...
local function update_device_profile()
  ...
  local updated_device_profile = t_utils.get_profile_definition("matter-camera.yml",
...
test.register_coroutine_test(
  ...
  function()
    ...
    for _, v in ipairs(cluster_to_component_map) do
      ...
      test.socket.capability:__expect_send(
        mock_device:generate_test_message(v.component, capabilities.audioVolume.volume(53))
      )
      test.socket.matter:__queue_receive({
        mock_device.id,
      ...
      test.socket.capability:__expect_send(
        mock_device:generate_test_message(v.component, capabilities.audioVolume.volume(9))
      )
    end
  end
)
...
test.register_coroutine_test(
  "Rate Distortion Trade Off Points reports should generate appropriate events",
  function()
    update_device_profile()
    test.wait_for_events()
    test.socket.matter:__queue_receive({
...
test.register_coroutine_test(
  ...
  function()
    ...
    test.socket.matter:__expect_send({
      mock_device.id, clusters.CameraAvStreamManagement.attributes.SpeakerVolumeLevel:write(mock_device, CAMERA_EP, 50)
    })
    test.socket.capability:__queue_receive({
      mock_device.id,
    ...
    test.socket.matter:__expect_send({
      mock_device.id, clusters.CameraAvStreamManagement.attributes.SpeakerVolumeLevel:write(mock_device, CAMERA_EP, 103)
    })
    test.socket.capability:__queue_receive({
      mock_device.id,
    ...
    test.socket.matter:__expect_send({
      mock_device.id, clusters.CameraAvStreamManagement.attributes.MicrophoneVolumeLevel:write(mock_device, CAMERA_EP, 166)
    })
    test.socket.capability:__queue_receive({
      mock_device.id,
  ...
  function()
    ...
    test.socket.capability:__queue_receive({
      ...
      { capability = "videoStreamSettings", component = "main", command = "setStream", args = {
        ...
        { upperLeftVertex = {x = 0, y = 1080}, lowerRightVertex = {x = 1920, y = 0} },
        "enabled",
        "disabled"
      }}
    })
