...
local function iso8061Timestamp(time)
  local t = os.date("!%Y-%m-%dT%TZ", time)
  print(t)
  return t
end
...
test.register_coroutine_test(
  "Report with power consumption after 15 minutes even when device is off", function()
    ...
    test.mock_time.advance_time(1000 * 60 * 15) -- Ensure that the timer created in create_poll_schedule triggers
