...
test.register_coroutine_test(
  ...
  function ()
    ...
    test.socket.capability:__expect_send(mock_device:generate_test_message("motion", capabilities.motionSensor.motion.active()))
    test.socket.matter:__queue_receive({
      mock_device.id,
      clusters.OccupancySensing.attributes.Occupancy:build_test_report_data(mock_device, 70, 0)
    })
    test.socket.capability:__expect_send(mock_device:generate_test_message("motion", capabilities.motionSensor.motion.inactive()))
  end
)
