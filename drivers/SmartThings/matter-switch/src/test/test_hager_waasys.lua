...
local descriptor = require "embedded_clusters.Descriptor"
...
local function add_matter_device(matter_device, parent, expected_profile_change)
  ...
  matter_device:expect_metadata_update({ profile = expected_profile_change })
...
local function add_switch_matter_device(matter_device, parent)
  test.mock_device.add_test_device(matter_device)
  test.socket.device_lifecycle:__queue_receive({ matter_device.id, "added" })
  test.socket.device_lifecycle:__queue_receive({ matter_device.id, "doConfigure" })
  matter_device:expect_metadata_update({ provisioning_state = "PROVISIONED" })
...
local function configure_parent(device)
  ...
  test.socket.matter:__expect_send({
    device.id,
    cluster_base.subscribe(device, 2, descriptor.ID, descriptor.attributes.PartsList.ID, nil)
  })
...
test.register_coroutine_test("Test: 4-Button Device Detection - Profile Changes from matter-bridge to 4-button When Four Button Endpoints Present", function()
  ...
  add_matter_device(matter_device, parent, "4-button")
  ...
  test.socket.matter:__expect_send({
    parent.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent, 0x08)
  })
  test.socket.matter:__expect_send({
    parent.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent, 0x09)
  })
  test.socket.matter:__expect_send({
    parent.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent, 0x0A)
  })
  test.socket.matter:__expect_send({
    parent.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent, 0x0B)
  })
  test.socket.matter:__queue_receive({
    parent.id,
...
test.register_coroutine_test("Test: Button Event Handling - Pushed, Double Press, and Held Events on 4-Button Device", function()
  ...
  initiate_info_changed(matter_device, "4-button")
  subscribe_switch_events(parent, { 8, 9, 10, 11 })
...
test.register_coroutine_test("Test: Device Type Handler - Handles Button (Type 15) and OnOff (Type 256) Device Types with Child Creation", function()
  ...
  test.socket.matter:__expect_send({
    parent.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent, 9)
  })
  test.socket.matter:__expect_send({
    parent.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent, 8)
  })
  test.socket.matter:__expect_send({
    parent.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent, 6)
  })
  ...
  test.wait_for_events()
  initiate_info_changed(matter_device, "2-button")
  subscribe_switch_events(parent, { 8, 9 })
  test.socket.capability:__expect_send(matter_device:generate_test_message("main", capabilities.button.supportedButtonValues({ "pushed", "double", "held" })))
  test.socket.capability:__expect_send(matter_device:generate_test_message("button2", capabilities.button.supportedButtonValues({ "pushed", "double", "held" })))

  test.wait_for_events()

  local child_switch = test.mock_device.build_test_child_device({
    profile = t_utils.get_profile_definition("light-binary.yml"),
    device_network_id = string.format("%s:6", parent.id),
    parent_device_id = parent.id,
    parent_assigned_child_key = "6"
  })
  test.wait_for_events()
  initiate_info_changed(child_switch, "light-binary")
...
test.register_coroutine_test("Test: 2G Relay - Profile Changes Between light-binary and 2-button Based On Endpoint Availability", function()
  ...
  test.timer.__create_and_queue_test_time_advance_timer(2, "oneshot")
  add_parent_device(parent)
  local relay = create_hager_2g_relay("light-binary", parent)
  add_switch_matter_device(relay, parent)
  test.wait_for_events()
  configure_parent(parent)
  test.wait_for_events()
  ...
  test.socket.matter:__expect_send({
    parent.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent, 3)
  })
  test.socket.matter:__expect_send({
    parent.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent, 4)
  })
  ...
  local child_switch = test.mock_device.build_test_child_device({
    profile = t_utils.get_profile_definition("light-binary.yml"),
    device_network_id = string.format("%s:4", parent.id),
    parent_device_id = parent.id,
    parent_assigned_child_key = "4"
  })

  initiate_info_changed(relay, "light-binary")
  test.socket.matter:__expect_send({
    parent.id,
    clusters.OnOff.attributes.OnOff:subscribe(parent, nil)
  })
  ...
  test.socket.matter:__expect_send({
    parent.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent, 12)
  })
  test.socket.matter:__expect_send({
    parent.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent, 13)
  })
  ...
  test.socket.matter:__expect_send({
    parent.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent, 10)
  })
  test.socket.matter:__expect_send({
    parent.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent, 11)
  })
...
test.register_coroutine_test("Test: Dimmer Device - Child Creation for Dimmable Endpoint with Button Support", function()
  ...
  add_parent_device(parent)
  local dimmer = create_hager_dimmer_device_2g("2-button", parent)
  ...
  test.socket.matter:__expect_send({
    parent.id,
    cluster_base.subscribe(parent, nil, clusters.OnOff.ID, clusters.OnOff.attributes.OnOff.ID, nil)
  })
  test.socket.matter:__expect_send({
    parent.id,
    cluster_base.subscribe(parent, nil, clusters.LevelControl.ID, clusters.LevelControl.attributes.CurrentLevel.ID, nil)
  })
  test.socket.matter:__expect_send({
    parent.id,
    cluster_base.subscribe(parent, nil, clusters.LevelControl.ID, clusters.LevelControl.attributes.MaxLevel.ID, nil)
  })
  test.socket.matter:__expect_send({
    parent.id,
    cluster_base.subscribe(parent, nil, clusters.LevelControl.ID, clusters.LevelControl.attributes.MinLevel.ID, nil)
  })
  ...
  test.socket.matter:__expect_send({
    parent.id,
    clusters.OnOff.server.commands.On(child_dimmer, 3)
  })
...
test.register_coroutine_test("Test: 1G Dimmer - Host Commands and Level Control Capabilities", function()
  ...
  add_matter_device(dimmer, parent_1g, "light-level")
  test.wait_for_events()
  ...
  test.socket.matter:__expect_send({
    parent_1g.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent_1g, 4)
  })
  ...
  initiate_info_changed(dimmer, "light-level")
  test.socket.matter:__expect_send({
    parent_1g.id,
    cluster_base.subscribe(parent_1g, nil, clusters.OnOff.ID, clusters.OnOff.attributes.OnOff.ID, nil)
  })
  test.socket.matter:__expect_send({
    parent_1g.id,
    cluster_base.subscribe(parent_1g, nil, clusters.LevelControl.ID, clusters.LevelControl.attributes.CurrentLevel.ID, nil)
  })
  test.socket.matter:__expect_send({
    parent_1g.id,
    cluster_base.subscribe(parent_1g, nil, clusters.LevelControl.ID, clusters.LevelControl.attributes.MaxLevel.ID, nil)
  })
  test.socket.matter:__expect_send({
    parent_1g.id,
    cluster_base.subscribe(parent_1g, nil, clusters.LevelControl.ID, clusters.LevelControl.attributes.MinLevel.ID, nil)
  })

  test.socket.capability:__queue_receive({ dimmer.id, { capability = "switch", component = "main", command = "on", args = {} } })
  dimmer.expect_native_cmd_handler_registration(dimmer, "switch", "on")

  test.socket.matter:__expect_send({
    parent_1g.id,
    clusters.OnOff.server.commands.On(dimmer, 4)
  })
  test.socket.capability:__queue_receive({ dimmer.id, { capability = "switchLevel", component = "main", command = "setLevel", args = { 20 } } })

  dimmer.expect_native_cmd_handler_registration(dimmer, "switchLevel", "setLevel")
...
test.register_coroutine_test("Test: PIR Device - Initialization with Motion and Illuminance Capabilities", function()
  ...
  add_matter_device(pir_device, parent_pir, "motion-illuminance")
  test.wait_for_events()
...
test.register_coroutine_test("Test: PIR Device - Complete Functionality with Motion, Illuminance, and Dimmer Support", function()
  ...
  test.socket.matter:__expect_send({
    parent_pir.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent_pir, 3)
  })
  test.socket.matter:__expect_send({
    parent_pir.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent_pir, 4)
  })
  test.socket.matter:__expect_send({
    parent_pir.id,
    clusters.Descriptor.attributes.DeviceTypeList:subscribe(parent_pir, 5)
  })
  ...
  test.socket.matter:__expect_send({
    parent_pir.id,
    cluster_base.subscribe(parent_pir, nil, clusters.OccupancySensing.ID, clusters.OccupancySensing.attributes.Occupancy.ID, nil)
  })
  ...
  test.socket.matter:__expect_send({
    parent_pir.id,
    cluster_base.subscribe(parent_pir, nil, clusters.LevelControl.ID, clusters.LevelControl.attributes.CurrentLevel.ID, nil)
  })
  test.socket.matter:__expect_send({
    parent_pir.id,
    cluster_base.subscribe(parent_pir, nil, clusters.LevelControl.ID, clusters.LevelControl.attributes.MaxLevel.ID, nil)
  })
  test.socket.matter:__expect_send({
    parent_pir.id,
  ...
  test.socket.matter:__queue_receive({
    parent_pir.id,
    clusters.OccupancySensing.attributes.Occupancy:build_test_report_data(parent_pir, 6, 1)
  })
...
test.register_coroutine_test("Test: Window Covering - Preference Changes for Reverse Polarity and Preset Position", function()
  ...
  local PRESET_LEVEL_KEY = child_wc.preferences.presetPosition
  assert(PRESET_LEVEL_KEY == "20", " presetPosition is set to 20")
