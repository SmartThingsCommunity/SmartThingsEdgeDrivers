...
-- Test: Helper functions contains_ep, resolve_host_and_ep, diff, extract
test.register_coroutine_test("Test: Helper functions contains_ep, resolve_host_and_ep, diff, extract", function()
    test.socket.matter:__set_channel_ordering("relaxed")

    -- Require the driver to access internal helper functions
    local driver = require "src/sub_drivers/Hager/init"

    -- Create subhub and host devices
    local subhub = create_subhub_device(0x0006)
    test.add_test_device(subhub)
    test.socket.device_lifecycle:__queue_receive({ subhub.id, "added" })
    test.socket.device_lifecycle:__queue_receive({ subhub.id, "init" })

    local host = create_host_device("matter-bridge", subhub)
    test.add_test_device(host)
    test.socket.device_lifecycle:__queue_receive({ host.id, "added" })
    test.socket.device_lifecycle:__queue_receive({ host.id, "init" })

    -- Manually link host and subhub (as add_host_device would do)
    host:set_field(SUBHUB_ID, subhub.id, { persist = true })
    host:set_field(HOST_ID, host.id, { persist = true })
    subhub:set_field(SUBHUB_ID, subhub.id, { persist = true })
    subhub:set_field(HOST_ID, host.id, { persist = true })

    -- Resolve host and subhub
    local resolved_subhub, resolved_ep = driver.resolve_host_and_ep(driver, host)
    assert(resolved_subhub == subhub, "resolve_host_and_ep returned wrong subhub")
    -- resolved_ep should be the main endpoint stored on the host (default is 0)
    assert(resolved_ep ~= nil, "resolve_host_and_ep did not return an endpoint")

    -- Test contains_ep
    assert(driver.contains_ep({3, 4}, 3) == true, "contains_ep failed to detect existing endpoint")
    assert(driver.contains_ep({3, 4}, 5) == false, "contains_ep incorrectly detected non‑existing endpoint")

    -- Test diff (added and removed endpoint detection)
    local added, removed = driver.diff({3, 4}, {4, 5})
    assert(#added == 1 and added[1] == 5, "diff added endpoint detection failed")
    assert(#removed == 1 and removed[1] == 3, "diff removed endpoint detection failed")

    -- Test extract helper – mock a PartsList report
    local mock_ib = {
        data = {
            elements = {
                data_types.Uint16(1),  -- should be filtered out
                data_types.Uint16(3),
                data_types.Uint16(4),
            }
        }
    }
    local extracted_eps = driver.extract(mock_ib)
    assert(#extracted_eps == 2 and extracted_eps[1] == 3 and extracted_eps[2] == 4,
           "extract failed to filter and return correct endpoints")
end)

-- Test: link_host_and_subhub and helper functions (contains_ep, diff, extract)
test.register_coroutine_test("Test: link_host_and_subhub and helper functions", function()
    -- Create subhub and host devices
    local subhub = create_subhub_device(0x0006)
    test.add_test_device(subhub)
    test.socket.device_lifecycle:__queue_receive({ subhub.id, "added" })
    ...
    local host = create_host_device("matter-bridge", subhub)
    -- add_host_device links host and subhub via link_host_and_subhub
    add_host_device(host, subhub)

    -- Verify link_host_and_subhub fields after init
    assert(host:get_field(SUBHUB_ID) == subhub.id, "Host SUBHUB_ID not set")
    assert(host:get_field(HOST_ID) == host.id, "Host HOST_ID not set")
    assert(subhub:get_field(SUBHUB_ID) == subhub.id, "Subhub SUBHUB_ID not set")
    assert(subhub:get_field(HOST_ID) == host.id, "Subhub HOST_ID not set")

    -- Trigger a PartsList report to test extract (filters out EP 1 & 2)
    test.socket.matter:__queue_receive({
        subhub.id,
        clusters.Descriptor.attributes.PartsList:build_test_report_data(
            subhub,
            0,
            data_types.Array({
                data_types.Uint16(1),
    ...
    -- Verify that ACTIVE_EPS on subhub contains only {3,4}
    local eps1 = subhub:get_field(ACTIVE_EPS)
    assert(eps1 ~= nil, "ACTIVE_EPS not set after first report")
    assert(#eps1 == 2 and eps1[1] == 3 and eps1[2] == 4, "extract failed to filter endpoints")

    -- Send a second PartsList report to test diff (added 5, removed 3)
    test.socket.matter:__queue_receive({
        subhub.id,
        clusters.Descriptor.attributes.PartsList:build_test_report_data(
            subhub,
    ...
    -- Verify that ACTIVE_EPS now reflects {4,5}
    local eps2 = subhub:get_field(ACTIVE_EPS)
    assert(eps2 ~= nil, "ACTIVE_EPS not set after second report")
    assert(#eps2 == 2 and eps2[1] == 4 and eps2[2] == 5, "diff failed to update endpoints")
end)

-- Test: resolve_host_and_ep and contains_ep via device_type_handler
test.register_coroutine_test("Test: resolve_host_and_ep and contains_ep", function()
    -- Create subhub and host devices
    local subhub = create_subhub_device(0x0006)
    test.add_test_device(subhub)
    test.socket.device_lifecycle:__queue_receive({ subhub.id, "added" })
    ...
    -- Set the main on/off endpoint for the host (default is endpoint 3 for Hager devices)
    host:set_field(FIELD_MAIN_ONOFF_EP, 3, { persist = true })

    -- Simulate a switch on command on the host; resolve_host_and_ep should map to subhub endpoint 3
    test.socket.capability:__queue_receive({ host.id, { capability = "switch", component = "main", command = "on", args = {} } })
    test.socket.matter:__expect_send({ subhub.id, clusters.OnOff.commands.On(subhub, 3) })
    test.wait_for_events()

    -- Trigger a DeviceTypeList report for a button endpoint (type 15) to test contains_ep logic
    test.socket.matter:__queue_receive({
        subhub.id,
        clusters.Descriptor.attributes.DeviceTypeList:build_test_report_data(subhub, 8, data_types.Array {
            { device_type = data_types.Uint32(15), revision = data_types.Uint16(1) }
        })
    })
    test.wait_for_events()

    local button_eps = subhub:get_field(BUTTON_EPS)
    assert(button_eps ~= nil and button_eps[1] == 8, "contains_ep failed to detect button endpoint")
end)
