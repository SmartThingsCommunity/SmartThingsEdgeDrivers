...
function SonosState:associate_device_record(device, info)
  ...
  if not household then
    log.error(string.format("No record of Sonos household for device %s",
  ...
  if not group then
    log.error(string.format("No record of Sonos group for device %s", (device.label or device.id or "<unknown device>")))
    return
  end
  ...
  if not player then
    log.error(string.format("No record of Sonos player for device %s", (device.label or device.id or "<unknown device>")))
    return
  end
  ...
  local websocket_url_changed = utils.update_field_if_changed(device, PlayerFields.WSS_URL, info.ssdp_info.wss_url,
    { persist = true })
  ...
  local household_id_changed = utils.update_field_if_changed(device, PlayerFields.HOUSEHOLD_ID, household.id,
    { persist = true })
  ...
  field_changed = utils.update_field_if_changed(device, PlayerFields.COORDINATOR_ID, group.coordinatorId,
    { persist = true })
  if field_changed then
...
function SonosState:get_coordinator_for_player(household_id, player_id)
  log.debug_with({ hub_logs = true },
    st_utils.stringify_table(
      { household_id = household_id, player_id = player_id }, "Get Coordinator For Player",
      true
    )
  )
...
function SonosState:get_coordinator_for_group(household_id, group_id)
  log.debug_with({ hub_logs = true },
    st_utils.stringify_table(
      { household_id = household_id, group_id = group_id }, "Get Coordinator For Group",
      true
    )
  )
  local household = _STATE.households[household_id]
  if household == nil then
    ...
    return nil,
        st_utils.stringify_table({ household = household },
...
function SonosState:get_sonos_ids_for_device(device)
  ...
  return sonos_objects.household.id, sonos_objects.group.id, sonos_objects.player.id
end
...
function SonosState:get_coordinator_for_device(device)
  ...
  if not household then
    return nil, nil,
        string.format("Couldn't determine Sonos Household for device %s",
          (device.label or device.id or "<unknown device>"))
  end
  local group = household.groups[group_id]
  if not group then
