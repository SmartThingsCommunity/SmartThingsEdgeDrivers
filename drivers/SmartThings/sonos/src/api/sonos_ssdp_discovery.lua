...
local function validate_sonos_ssdp_response(response)
  local ip =
      response:get_one("location"):match("http://([^,/]+):[^/]+/.+%.xml")
  ...
  return has_server and ip and is_group_coordinator and group_id and group_name and household_id and wss_url and
      expires_in and tonumber(expires_in) and player_id and (redundant_st == sonos_ssdp.SearchTerm) and true
end
...
local function post_process_response(response)
  ...
  local ret = setmetatable({
    ...
    player_id = player_id
  }, { __tostring = function(tbl) return st_utils.stringify_table(tbl, nil, true) end })
  return ret
end
...
sonos_ssdp.required_headers = { "usn", "server", "location", "groupinfo.smartspeaker.audio", "websock.smartspeaker.audio",
...
function sonos_ssdp.ssdp_info_eq(a, b)
  return (a.group_id == b.group_id) and
      ...
      (a.household_id == b.household_id) and
      (a.ip == b.ip) and
      (a.is_group_coordinator == b.is_group_coordinator) and
...
function SonosPersistentSsdpTask:get_player_info(reply_tx, ...)
  ...
  if player_id ~= nil and type(player_id) == "string" then
    ...
    lookup_key = string.format("%s/%s", household_id_or_mac, player_id)
  else
...
function sonos_ssdp.spawn_persistent_ssdp_task()
  ...
  local callback = function(sonos_ssdp_info)
    local unique_key = string.format("%s/%s", sonos_ssdp_info.household_id, sonos_ssdp_info.player_id)
    local maybe_known = task_handle.player_info_by_sonos_ids[unique_key]
    local is_new_information = not (maybe_known and maybe_known.ssdp_info.expires_at > os.time() and sonos_ssdp.ssdp_info_eq(maybe_known.ssdp_info, sonos_ssdp_info))

    local info_to_send

    if is_new_information then
      local headers = SonosApi.make_rest_headers()
      local discovery_info, err = SonosApi.RestApi.get_player_info(sonos_ssdp_info.ip, SonosApi.DEFAULT_SONOS_PORT,
        headers)
      ...
      info_to_send = { ssdp_info = maybe_known.ssdp_info, discovery_info = maybe_known.discovery_info, force_refresh = false }
    end
  ...
  local thread = cosock.spawn(make_persistent_task_impl(ssdp_search_handle, control_rx, status_tx, callback),
