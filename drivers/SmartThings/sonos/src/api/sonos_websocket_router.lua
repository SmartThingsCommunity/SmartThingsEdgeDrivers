            ...
            for _, uuid in ipairs(listener_ids_for_socket[recv.id]) do
              ...
              if listener ~= nil then
                if listener.device and listener.device.label then
                  log.debug(string.format("SonosConnection for device %s handling websocket message",
...
local function _make_websocket(url_table, api_key)
  ...
  local config = LustreConfig.default():header("X-Sonos-Api-Key", api_key)
...
function SonosWebSocketRouter.register_listener_for_socket(listener,
  ...
  if listener and listener.device and listener.device.label then
    log.debug("Registering SonosConnection for device %s as listener for player %s websocket", listener.device
      .label, unique_key_for_socket)
  end
...
function SonosWebSocketRouter.open_socket_for_player(household_id, player_id, wss_url, api_key)
  local unique_key = string.format("%s/%s", household_id, player_id)
  if not websockets[unique_key] then
    ...
    if err or not wss then
      return nil, string.format(
        "Could not create websocket connection for %s: %s", unique_key,
        err)
    else
...
function SonosWebSocketRouter.cleanup_unused_sockets(driver)
  ...
  for _, device in ipairs(known_devices) do
    ...
    local coordinator_unique_key = string.format("%s/%s", household_id, coordinator_id)
