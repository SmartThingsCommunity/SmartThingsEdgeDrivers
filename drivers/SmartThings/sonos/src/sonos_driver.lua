...
---@field private oauth { token: {accessToken: string, expiresAt: number}, endpoint_app_info: { state: "connected"|"disconnected" } } cached OAuth info
...
function SonosDriver:get_device_by_dni(dni)
  ...
  if cached_device_id then return self:get_device_info(cached_device_id) end
...
function SonosDriver:handle_augmented_data_change(update_key, decoded)
  if update_key == "endpointAppInfo" then
    log.debug(st_utils.stringify_table(decoded, "upsert endpoint app info", true))
...
function SonosDriver:handle_augmented_store_upsert(update_key, update_value)
  ...
  if not decode_success then
    log.warn(
      string.format("Unable to decode augmented data update payload: %s",
...
function SonosDriver:notify_augmented_data_changed(update_kind, update_key, update_value)
  local already_connected =
      self.oauth and
      self.oauth.endpoint_app_info and
      self.oauth.endpoint_app_info.state == "connected"
  ...
  elseif update_kind == "upsert" then
    ...
    log.debug(st_utils.stringify_table({ kind = update_kind, key = update_key, value = "..." },
      "unexpected event kind in augmented data change event", true))
  end

  if
      self.oauth.endpoint_app_info and
      self.oauth.endpoint_app_info.state == "connected" and
...
function SonosDriver:request_oauth_token()
  ...
  if not result then
    return nil,
        string.format("Error requesting OAuth token via Security API: %s", err)
  end
...
function SonosDriver:get_oauth_token()
  local decode_success, maybe_token = pcall(json.decode, self.hub_augmented_driver_data.sonosOAuthToken)
  if
      decode_success and
      type(maybe_token) == "table" and
      type(maybe_token.accessToken) == "string" and
      type(maybe_token.expiresAt) == "number"
  then
...
local function make_ssdp_event_handler(driver, discovery_event_subscription, oauth_token_subscription)
  return function()
    local unauthorized = {}
    local discovered = {}
    while true do
      local recv_ready, _, select_err = cosock.socket.select({ discovery_event_subscription, oauth_token_subscription },
        nil, nil)
        ...
        for _, receiver in ipairs(recv_ready) do
          if receiver == oauth_token_subscription then
            ...
            if not token_evt then
              ...
              log.debug(string.format("token event: %s", st_utils.stringify_table(token_evt)))
          ...
          if receiver == discovery_event_subscription then
            ...
            if event then
              local unique_key = string.format("%s/%s", event.ssdp_info.household_id, event.discovery_info.playerId)
              if event.force_refresh or (not (unauthorized[unique_key] or discovered[unique_key])) then
                local success, handle_err, err_code = driver:handle_player_discovery_info(event)
                if not success then
...
function SonosDriver:start_ssdp_event_task()
  ...
  if ssdp_task then
    ...
    self.ssdp_event_thread_handle = cosock.spawn(make_ssdp_event_handler(self, ssdp_task_subscription,
...
function SonosDriver:handle_player_discovery_info(info, device)
  local sw_gen = info.discovery_info.device.swGen
  local rest_url = net_url.parse(info.discovery_info.restUrl)
  local use_legacy_api_key = (type(sw_gen) == "number" and sw_gen < 2)

  local maybe_token, no_token_reason = self:get_oauth_token()

  local headers = SonosApi.make_rest_headers(use_legacy_api_key, (maybe_token and maybe_token.accessToken) or nil)
  ...
  if response and response._objectType == "globalError" then
    ...
    local error_string = string.format("`getGroups` response error for player \"%s\":\n\tError Code: %s",
  ...
  if response and response._objectType ~= "groups" then
    return nil,
        string.format("Unexpected response type to group info request: %s",
  ...
  local maybe_device_id = self.sonos:get_device_id_for_player(info.ssdp_info.household_id,
    info.discovery_info.playerId)
  ...
  if device_to_update then
    ...
    local name = info.discovery_info.device.name or info.discovery_info.device.modelDisplayName or "Unknown Sonos Player"
    ...
    -- log.debug(st_utils.stringify_table(try_create_message, "try_create_device", true))
