...
local do_refresh = function(self, device)
  ...
  device:send(custom_read_attribute(device, Basic.ID, PREF_ATTR_ID))
end
...
local function device_added(driver, device)
  ...
  device:send(cluster_base.write_manufacturer_specific_attribute(device, Basic.ID, PREF_ATTR_ID, MFG_CODE,
            data_types.CharString, "\x00\x02\x00\x00\x00\x00\x00"))
  device:send(cluster_base.write_manufacturer_specific_attribute(device, Basic.ID, PREF_ATTR_ID, MFG_CODE,
            data_types.CharString, "\x00\x08\x00\x00\x00\x00\x00"))
end

-- this is called when a device info is changed.
  ...
  zigbee_handlers = {
    attr = {
      ...
      [Basic.ID] = {
        ...
        [PREF_ATTR_ID] = pref_attr_handler,
      },
      [0x000D] = {
