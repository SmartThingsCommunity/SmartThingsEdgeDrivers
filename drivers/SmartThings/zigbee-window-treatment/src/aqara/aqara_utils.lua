...
local function SinglePrecisionFloatConversion(input)
  input = utils.round(input)
  if input > 100 then
    input = 100
  elseif input == 0 then
    return SinglePrecisionFloat(0, -127, 0)
  end
  local sign = input < 0 and 1 or 0
  local value = math.abs(input)
  local exponent = 0
  while value >= 2 do
      value = value / 2
      exponent = exponent + 1
  ...
  local result = SinglePrecisionFloat(sign, exponent, mantissa)
  return result
end
...
aqara_utils.SinglePrecisionFloatConversion = SinglePrecisionFloatConversion
