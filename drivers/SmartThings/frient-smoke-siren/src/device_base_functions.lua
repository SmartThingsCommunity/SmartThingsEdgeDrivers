  ...
  for _, fingerprint in ipairs(FRIENT_DEVICE_FINGERPRINTS) do
    if device:get_manufacturer() == fingerprint.mfr and device:get_model() == fingerprint.model then
      if device:supports_capability(capabilities.battery) then
        battery_defaults.build_linear_voltage_init(2.8, 3.0)(driver, device)
      end
...
function BASE_FUNCTIONS.do_configure(driver, device, event, args)
  ...
  for _, fingerprint in ipairs(FRIENT_DEVICE_FINGERPRINTS) do
    if device:get_manufacturer() == fingerprint.mfr and device:get_model() == fingerprint.model then
      ...
      if fingerprint.ENDPOINT_TAMPER then
        --device:send(device_management.build_bind_request(device, zcl_clusters.IASZone.ID, driver.environment_info.hub_zigbee_eui, fingerprint.ENDPOINT_TAMPER))
        --device:send(IASZone.attributes.IASCIEAddress:write(device, data_types.IeeeAddress(driver.environment_info.hub_zigbee_eui)):to_endpoint(fingerprint.ENDPOINT_TAMPER))
        device:send(IASZone.attributes.ZoneStatus:configure_reporting(device, 0, 6*60*60, 1):to_endpoint(fingerprint.ENDPOINT_TAMPER))
      end
      if fingerprint.ENDPOINT_TEMPERATURE then
        ...
        device:send(TemperatureMeasurement.attributes.MeasuredValue:configure_reporting(device, 30, 1*60*60, sensitivity):to_endpoint(fingerprint.ENDPOINT_TEMPERATURE))
      end

      -- Check if we have the software version
      local sw_version = device:get_field(BASE_FUNCTIONS.PRIMARY_SW_VERSION)
      if ((sw_version == nil) or (sw_version == "")) then
        log.warn("Configure: Firmware version not detected, checking software version")
        device:send(cluster_base.read_manufacturer_specific_attribute(device, Basic.ID, BASE_FUNCTIONS.DEVELCO_BASIC_PRIMARY_SW_VERSION_ATTR, BASE_FUNCTIONS.DEVELCO_MANUFACTURER_CODE))
      else
        log.trace("Configure: Firmware version: 0x"..sw_version)
      end
    end
  end
end
