...
--- @class st.matter.clusters.DiscoBall.Stopped
--- @alias Stopped
---
--- @field public ID number 0x0001 the ID of this event
--- @field public NAME string "Stopped" the name of this event
local Stopped = {
  ID = 0x0001,
  NAME = "Stopped",
...
function Stopped:augment_type(base_type_obj)
  local elems = {}
...
function Stopped:read(device, endpoint_id)
...
--- @param endpoint_id number|nil
--- @return st.matter.interaction_model.InteractionRequest
function Stopped:subscribe(device, endpoint_id)
...
--- Builds an Stopped test event report reponse to be used by
...
function Stopped:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "StoppedEventData", ID = 0x15}))
...
function Stopped:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function Stopped:set_parent_cluster(cluster)
