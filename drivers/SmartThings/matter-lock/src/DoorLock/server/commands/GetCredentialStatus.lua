...
local GetCredentialStatus = {}

GetCredentialStatus.NAME = "GetCredentialStatus"
GetCredentialStatus.ID = 0x0024
GetCredentialStatus.field_defs = {
  {
    name = "credential",
    field_id = 0,
    is_nullable = false,
...
function GetCredentialStatus:init(device, endpoint_id, credential)
  local out = {}
  ...
  setmetatable(out, {
    __index = GetCredentialStatus,
    __tostring = GetCredentialStatus.pretty_print
  })
...
function GetCredentialStatus:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

function GetCredentialStatus:augment_type(base_type_obj)
  local elems = {}
...
function GetCredentialStatus:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(GetCredentialStatus, {__call = GetCredentialStatus.init})
