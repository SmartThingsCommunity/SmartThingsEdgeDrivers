...
local ClearAliroReaderConfig = {}

ClearAliroReaderConfig.NAME = "ClearAliroReaderConfig"
ClearAliroReaderConfig.ID = 0x0029
ClearAliroReaderConfig.field_defs = {
}

function ClearAliroReaderConfig:build_test_command_response(device, endpoint_id, status)
...
function ClearAliroReaderConfig:init(device, endpoint_id)
  local out = {}
  local args = {}
  ...
  setmetatable(out, {
    __index = ClearAliroReaderConfig,
    __tostring = ClearAliroReaderConfig.pretty_print
  })
...
function ClearAliroReaderConfig:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

function ClearAliroReaderConfig:augment_type(base_type_obj)
  local elems = {}
...
function ClearAliroReaderConfig:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(ClearAliroReaderConfig, {__call = ClearAliroReaderConfig.init})
