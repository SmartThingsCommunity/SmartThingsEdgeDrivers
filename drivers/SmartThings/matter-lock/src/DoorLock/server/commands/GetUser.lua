...
function GetUser:init(device, endpoint_id, user_index)
  local out = {}
  ...
  setmetatable(out, {
    __index = GetUser,
    __tostring = GetUser.pretty_print
  })
...
function GetUser:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

function GetUser:augment_type(base_type_obj)
  local elems = {}
...
function GetUser:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(GetUser, {__call = GetUser.init})
