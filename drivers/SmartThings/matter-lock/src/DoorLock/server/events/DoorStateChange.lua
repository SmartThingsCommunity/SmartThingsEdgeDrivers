...
local DoorStateChange = {
  ID = 0x0001,
  NAME = "DoorStateChange",
  base_type = data_types.Structure,
}

DoorStateChange.field_defs = {
  {
    name = "door_state",
    field_id = 0,
    is_nullable = false,
    is_optional = false,
    data_type = require "DoorLock.types.DoorStateEnum",
  },
}

function DoorStateChange:augment_type(base_type_obj)
  local elems = {}
...
function DoorStateChange:read(device, endpoint_id)
...
function DoorStateChange:subscribe(device, endpoint_id)
...
function DoorStateChange:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "DoorStateChangeEventData", ID = 0x15}))
...
function DoorStateChange:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function DoorStateChange:set_parent_cluster(cluster)
