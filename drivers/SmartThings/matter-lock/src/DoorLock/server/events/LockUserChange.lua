...
local LockUserChange = {
  ID = 0x0004,
  NAME = "LockUserChange",
  base_type = data_types.Structure,
}

LockUserChange.field_defs = {
  {
    name = "lock_data_type",
    field_id = 0,
    is_nullable = false,
    is_optional = false,
    data_type = require "DoorLock.types.LockDataTypeEnum",
  },
  {
    name = "data_operation_type",
    field_id = 1,
    is_nullable = false,
    is_optional = false,
...
function LockUserChange:augment_type(base_type_obj)
  local elems = {}
...
function LockUserChange:read(device, endpoint_id)
...
function LockUserChange:subscribe(device, endpoint_id)
...
function LockUserChange:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "LockUserChangeEventData", ID = 0x15}))
...
function LockUserChange:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function LockUserChange:set_parent_cluster(cluster)
