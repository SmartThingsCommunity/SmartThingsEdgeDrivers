...
DoorLockAlarm.field_defs = {
  {
    name = "alarm_code",
    field_id = 0,
    is_nullable = false,
    is_optional = false,
    data_type = require "DoorLock.types.AlarmCodeEnum",
  },
}

function DoorLockAlarm:augment_type(base_type_obj)
  local elems = {}
...
function DoorLockAlarm:read(device, endpoint_id)
...
function DoorLockAlarm:subscribe(device, endpoint_id)
...
function DoorLockAlarm:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "DoorLockAlarmEventData", ID = 0x15}))
...
function DoorLockAlarm:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function DoorLockAlarm:set_parent_cluster(cluster)
