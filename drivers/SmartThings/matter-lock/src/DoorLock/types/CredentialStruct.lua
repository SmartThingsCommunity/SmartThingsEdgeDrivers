...
local CredentialStruct = {}
local new_mt = StructureABC.new_mt({NAME = "CredentialStruct", ID = data_types.name_to_id_map["Structure"]})

CredentialStruct.field_defs = {
  {
    name = "credential_type",
    field_id = 0,
    is_nullable = false,
    is_optional = false,
    data_type = require "DoorLock.types.CredentialTypeEnum",
  },
  {
...
CredentialStruct.init = function(cls, tbl)
    local o = {}
    o.elements = {}
    o.num_elements = 0
...
CredentialStruct.serialize = function(self, buf, include_control, tag)
  return data_types['Structure'].serialize(self.elements, buf, include_control, tag)
end

new_mt.__call = CredentialStruct.init
new_mt.__index.serialize = CredentialStruct.serialize

CredentialStruct.augment_type = function(self, val)
  local elems = {}
...
setmetatable(CredentialStruct, new_mt)
