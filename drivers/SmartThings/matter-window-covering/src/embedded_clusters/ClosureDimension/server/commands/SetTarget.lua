...
function SetTarget:build_test_command_response(device, endpoint_id, status)
...
function SetTarget:init(device, endpoint_id, position, latch, speed)
  local out = {}
  ...
  setmetatable(out, {
    __index = SetTarget,
    __tostring = SetTarget.pretty_print
  })
...
function SetTarget:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

function SetTarget:augment_type(base_type_obj)
  local elems = {}
...
function SetTarget:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(SetTarget, {__call = SetTarget.init})
