...
Calibrate.ID = 0x0002
Calibrate.field_defs = {
}

function Calibrate:build_test_command_response(device, endpoint_id, status)
...
function Calibrate:init(device, endpoint_id)
  local out = {}
  local args = {}
  ...
  setmetatable(out, {
    __index = Calibrate,
    __tostring = Calibrate.pretty_print
  })
...
function Calibrate:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

function Calibrate:augment_type(base_type_obj)
  local elems = {}
...
function Calibrate:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(Calibrate, {__call = Calibrate.init})
