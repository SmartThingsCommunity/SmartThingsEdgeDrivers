...
local function update_profile_table(device)
  ...
  for _, entry in pairs(feature_table) do
    device.log.info_with({hub_logs=true}, string.format("!!! endpoint: %s, has_lift: %s, has_tilt: %s !!!", entry.endpoint, entry.has_lift, entry.has_tilt))
    profile_table[entry.endpoint] = "window-covering"
    if entry.has_tilt then
      device.log.info_with({hub_logs=true}, string.format("!!! window-covering-tilt !!!"))
      profile_table[entry.endpoint] = "window-covering-tilt"
      if not entry.has_lift then
        device.log.info_with({hub_logs=true}, string.format("!!! window-covering-tilt-only !!!"))
...
local function create_or_update_multi_devices(driver, device, wc_cluster_eps)
  ...
  for device_num, ep_id in ipairs(wc_cluster_eps) do
    if ep_id ~= default_endpoint_id then -- don't create a child device that maps to the main endpoint
      ...
      device.log.info_with({hub_logs=true}, string.format("!!! do_create_or_update: endpoint: %s, profile: %s !!!", ep_id, child_profile))
...
local function device_added(driver, device)
  device:send(clusters.WindowCovering.attributes.OperationalStatus:read())
  device:emit_event(
