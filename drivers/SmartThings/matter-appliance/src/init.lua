...
local function temperature_setpoint_attr_handler(driver, device, ib, response)
  ...
  local min = device:get_field(common_utils.setpoint_limit_device_field.MIN_TEMP) or 0
  local max = device:get_field(common_utils.setpoint_limit_device_field.MAX_TEMP) or 100
  local unit = "C"
  local range = {
    minimum = min,
    maximum = max,
