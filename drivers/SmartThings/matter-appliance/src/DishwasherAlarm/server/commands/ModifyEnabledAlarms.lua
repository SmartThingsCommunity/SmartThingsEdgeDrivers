...
local ModifyEnabledAlarms = {}

ModifyEnabledAlarms.NAME = "ModifyEnabledAlarms"
ModifyEnabledAlarms.ID = 0x0001
ModifyEnabledAlarms.field_defs = {
  {
    name = "mask",
    field_id = 0,
    is_nullable = false,
...
function ModifyEnabledAlarms:build_test_command_response(device, endpoint_id, status)
...
function ModifyEnabledAlarms:init(device, endpoint_id, mask)
  local out = {}
  local args = {mask}
  ...
  setmetatable(out, {
    __index = ModifyEnabledAlarms,
    __tostring = ModifyEnabledAlarms.pretty_print
  })
...
function ModifyEnabledAlarms:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

function ModifyEnabledAlarms:augment_type(base_type_obj)
  local elems = {}
...
function ModifyEnabledAlarms:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(ModifyEnabledAlarms, {__call = ModifyEnabledAlarms.init})
