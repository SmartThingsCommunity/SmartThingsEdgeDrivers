...
command_mt.__index = function(self, key)
  if command_mt.__command_cache[key] == nil then
    local req_loc = string.format("EnergyEvseMode.server.commands.%s", key)
...
local EnergyEvseModeServerCommands = {}

function EnergyEvseModeServerCommands:set_parent_cluster(cluster)
