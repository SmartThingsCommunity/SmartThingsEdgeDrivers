...
attr_mt.__index = function(self, key)
  if attr_mt.__attr_cache[key] == nil then
    local req_loc = string.format("EnergyEvseMode.server.attributes.%s", key)
...
local EnergyEvseModeServerAttributes = {}

function EnergyEvseModeServerAttributes:set_parent_cluster(cluster)
