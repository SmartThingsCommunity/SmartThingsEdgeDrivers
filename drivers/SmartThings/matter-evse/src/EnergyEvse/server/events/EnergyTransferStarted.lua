...
EnergyTransferStarted.field_defs = {
  {
...
function EnergyTransferStarted:augment_type(base_type_obj)
  local elems = {}
...
function EnergyTransferStarted:read(device, endpoint_id)
...
function EnergyTransferStarted:subscribe(device, endpoint_id)
...
function EnergyTransferStarted:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "EnergyTransferStartedEventData", ID = 0x15}))
...
function EnergyTransferStarted:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function EnergyTransferStarted:set_parent_cluster(cluster)
