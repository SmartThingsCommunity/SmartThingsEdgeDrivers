...
local StartDiagnostics = {}

StartDiagnostics.NAME = "StartDiagnostics"
StartDiagnostics.ID = 0x0004
StartDiagnostics.field_defs = {
}

function StartDiagnostics:build_test_command_response(device, endpoint_id, status)
...
function StartDiagnostics:init(device, endpoint_id)
  local out = {}
  local args = {}
  ...
  setmetatable(out, {
    __index = StartDiagnostics,
    __tostring = StartDiagnostics.pretty_print
  })
...
function StartDiagnostics:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

function StartDiagnostics:augment_type(base_type_obj)
  local elems = {}
...
function StartDiagnostics:deserialize(tlv_buf)
  return TLVParser.decode_tlv(tlv_buf)
end

setmetatable(StartDiagnostics, {__call = StartDiagnostics.init})
