...
GetTargets.NAME = "GetTargets"
GetTargets.ID = 0x0006
GetTargets.field_defs = {}

function GetTargets:init(device, endpoint_id)
  local out = {}
  local args = {}
  ...
  setmetatable(out, {
    __index = GetTargets,
    __tostring = GetTargets.pretty_print
  })
...
function GetTargets:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

function GetTargets:augment_type(base_type_obj)
  local elems = {}
...
function GetTargets:deserialize(tlv_buf)
  return TLVParser.decode_tlv(tlv_buf)
end

setmetatable(GetTargets, {__call = GetTargets.init})
