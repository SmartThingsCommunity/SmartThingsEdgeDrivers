...
local MeasurementRangeStruct = require "ElectricalPowerMeasurement.types.MeasurementRangeStruct"

local MeasurementPeriodRanges = {
  ID = 0x0000,
  NAME = "MeasurementPeriodRanges",
  base_type = data_types.Structure,
}

MeasurementPeriodRanges.field_defs = {
  {
...
function MeasurementPeriodRanges:augment_type(base_type_obj)
  local elems = {}
...
function MeasurementPeriodRanges:read(device, endpoint_id)
...
function MeasurementPeriodRanges:subscribe(device, endpoint_id)
...
function MeasurementPeriodRanges:build_test_event_report(
  device,
  endpoint_id,
  fields,
  status
  ...
  data.elements = {}
  data.num_elements = 0
  setmetatable(data, StructureABC.new_mt({NAME = "MeasurementPeriodRangesEventData", ID = 0x15}))
...
function MeasurementPeriodRanges:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

function MeasurementPeriodRanges:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

return MeasurementPeriodRanges
