...
--- @class st.matter.clusters.PressureMeasurement.ScaledValue
--- @alias ScaledValue
---
...
function ScaledValue:read(device, endpoint_id)
...
--- Reporting policy: ScaledValue => true => mandatory
...
--- @param endpoint_id number|nil
--- @return any
function ScaledValue:subscribe(device, endpoint_id)
...
--- Builds an ScaledValue test attribute reponse for the driver integration testing framework
---
...
function ScaledValue:build_test_report_data(
  device,
  endpoint_id,
  value,
  status
...
function ScaledValue:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)

  return data
end

setmetatable(ScaledValue, {__call = ScaledValue.new_value, __index = ScaledValue.base_type})
return ScaledValue
