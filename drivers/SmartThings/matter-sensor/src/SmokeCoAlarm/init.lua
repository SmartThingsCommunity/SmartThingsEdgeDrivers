...
do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.SmokeCoAlarm.server.attributes")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      SmokeCoAlarm.attribute_direction_map[alias] = "server"
    end
  end
end
...
do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.SmokeCoAlarm.server.commands")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      SmokeCoAlarm.command_direction_map[alias] = "server"
    end
  end
end

do
  local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.SmokeCoAlarm.client.commands")
  if has_aliases then
    for alias, _ in pairs(aliases) do
      SmokeCoAlarm.command_direction_map[alias] = "client"
    end
  end
end
