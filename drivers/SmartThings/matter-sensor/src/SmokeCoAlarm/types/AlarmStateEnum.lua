...
local new_mt = UintABC.new_mt({NAME = "AlarmStateEnum", ID = data_types.name_to_id_map["Uint8"]}, 1)
...
local has_aliases, aliases = pcall(require, "st.matter.clusters.aliases.SmokeCoAlarm.types.AlarmStateEnum")
if has_aliases then
  aliases:add_to_class(AlarmStateEnum)
end
