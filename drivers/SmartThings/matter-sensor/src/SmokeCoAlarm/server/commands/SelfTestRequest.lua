...
local SelfTestRequest = {}

SelfTestRequest.NAME = "SelfTestRequest"
SelfTestRequest.ID = 0x0000
SelfTestRequest.field_defs = {
}

function SelfTestRequest:build_test_command_response(device, endpoint_id, status)
...
function SelfTestRequest:init(device, endpoint_id)
  local out = {}
  local args = {}
  ...
  setmetatable(out, {
    __index = SelfTestRequest,
    __tostring = SelfTestRequest.pretty_print
  })
...
function SelfTestRequest:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

function SelfTestRequest:augment_type(base_type_obj)
  local elems = {}
...
function SelfTestRequest:deserialize(tlv_buf)
  local data = TLVParser.decode_tlv(tlv_buf)
  self:augment_type(data)
  return data
end

setmetatable(SelfTestRequest, {__call = SelfTestRequest.init})
