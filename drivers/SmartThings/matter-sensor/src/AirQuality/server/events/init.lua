...
event_mt.__index = function(self, key)
  if event_mt.__event_cache[key] == nil then
    local req_loc = string.format("AirQuality.server.events.%s", key)
...
local AirQualityEvents = {}

function AirQualityEvents:set_parent_cluster(cluster)
  self._cluster = cluster
  return self
end

setmetatable(AirQualityEvents, event_mt)
