...
test.register_coroutine_test(
  ...
  function()
    ...
    test.socket.device_lifecycle:__queue_receive({ mock_inovelli_vzw31_sn.id, "added" })

    for button_name, _ in pairs(mock_inovelli_vzw31_sn.profile.components) do
  ...
  {
    {
      channel = "zwave",
      direction = "receive",
      message = { mock_inovelli_vzw31_sn.id, zw_test_utils.zwave_test_build_receive_command(CentralScene:Notification({
        scene_number = 1,
    ...
    },
    {
      channel = "capability",
      direction = "send",
      message = mock_inovelli_vzw31_sn:generate_test_message("button1", capabilities.button.button.pushed({
        state_change = true
      }))
    },
  },
...
test.register_message_test(
  "Central scene notification button2 pressed 4 times should emit button events",
  {
    ...
    },
    {
      channel = "capability",
      direction = "send",
      message = mock_inovelli_vzw31_sn:generate_test_message("button2", capabilities.button.button.pushed_4x({
        state_change = true
      }))
    },
  },
...
test.register_message_test(
  "Refresh capability should request switch level and meter data",
  {
    {
      channel = "capability",
      direction = "receive",
