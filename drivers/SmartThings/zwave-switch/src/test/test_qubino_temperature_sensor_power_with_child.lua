...
local qubino_temperature_sensor_with_power_with_child = {
  {
    command_classes = {
      {value = zw.METER},
      {value = zw.BASIC},
...
local mock_device = test.mock_device.build_test_zwave_device({
  ...
  zwave_endpoints = qubino_temperature_sensor_with_power_with_child,
...
test.register_message_test(
  "Switch Binary report from endpoint 2 should be handled by switch2 component",
  {
    {
      channel = "zwave",
      direction = "receive",
...
test.register_message_test(
  "Celsius temperature reports should be handled",
  {
    ...
    },
    {
      channel = "capability",
      direction = "send",
      message = mock_device:generate_test_message("extraTemperatureSensor", capabilities.temperatureMeasurement.temperature({ value = 21.5, unit = 'C' }))
    }
  }
)
...
test.register_coroutine_test(
  "extraTemperatureSensor should be deleted or created depending on temperature reports",
  function()
    test.timer.__create_and_queue_test_time_advance_timer(1, "oneshot")
    test.socket.capability:__set_channel_ordering("relaxed")
    test.socket.zwave:__set_channel_ordering("relaxed")
    ...
    -- test.wait_for_events()
    -- assert(utils.table_size(mock_device.profile.components) == 2, "extraTemperatureSensor should be deleted after receiving wrong temperature value")
    ...
    -- test.wait_for_events()
    -- assert(utils.table_size(mock_device.profile.components) == 3, "extraTemperatureSensor should be created after receiving correct temperature value")

    test.socket.capability:__expect_send(
      mock_device:generate_test_message("extraTemperatureSensor", capabilities.temperatureMeasurement.temperature({ value = 21.5, unit = 'C' }))
    )
  end
)
