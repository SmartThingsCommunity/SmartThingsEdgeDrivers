    ...
    function()
      ...
      for voltage, batt_perc in pairs(battery_test_map) do
        test.socket.zigbee:__queue_receive({
          mock_device.id,
          PowerConfiguration.attributes.MainsVoltage:build_test_attr_report(mock_device, voltage)
        })
...
test.register_message_test(
  "Driver should poll device at the inclusion",
  {
    ...
    },
    {
      ...
      message = {
        mock_device.id,
        PowerConfiguration.attributes.MainsVoltage:read(mock_device)
      }
    },
    {
      channel = "zigbee",
...
test.register_coroutine_test(
    ...
    function ()
      ...
      test.socket.zigbee:__expect_send({
                                         mock_device.id,
                                         RelativeHumidity.attributes.MeasuredValue:configure_reporting(mock_device, 10, 610, 6400)
                                       })
      test.socket.zigbee:__expect_send({
                                         mock_device.id,
      ...
      test.socket.zigbee:__expect_send({
                                         mock_device.id,
                                         TemperatureMeasurement.attributes.MeasuredValue:configure_reporting(mock_device, 1, 0, 3200)
                                       })
      test.socket.zigbee:__expect_send({
                                         mock_device.id,
      ...
      test.socket.zigbee:__expect_send({
                                         mock_device.id,
                                         PowerConfiguration.attributes.MainsVoltage:configure_reporting(mock_device, 0x0C, 0, 500)
                                       })
      test.socket.zigbee:__expect_send({
                                         mock_device.id,
