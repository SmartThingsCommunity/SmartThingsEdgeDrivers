...
test.register_coroutine_test(
  "doConfigure should send SoundSwitch ConfigurationSet to tamper sound component",
  function()
    ...
    test.socket.zwave:__expect_send(
      zw_test_utils.zwave_test_build_send_command(
        mock_siren,
        SoundSwitch:ConfigurationSet(
          {default_tone_identifier = 17, volume = 5},
...
test.register_coroutine_test(
  "added lifecycle should clear tamper and clear all sound component alarm and chime states",
  function()
    test.socket.capability:__set_channel_ordering("relaxed")
    ...
    -- clearAlarmAndChimeStateOfSoundComponents: endpoints 1-8
    test.socket.capability:__expect_send(
      mock_siren:generate_test_message("main", capabilities.alarm.alarm.off())
    )
    test.socket.capability:__expect_send(
    ...
    for i = 2, 8 do
      test.socket.capability:__expect_send(
        mock_siren:generate_test_message("sound"..i, capabilities.alarm.alarm.off())
      )
      test.socket.capability:__expect_send(
        mock_siren:generate_test_message("sound"..i, capabilities.chime.chime.off())
      )
    end
  end
)
...
test.register_coroutine_test(
  "HOME_SECURITY TAMPERING_PRODUCT_MOVED should trigger tamper detected and auto-clear after 5 second delay",
  function()
    test.timer.__create_and_queue_test_time_advance_timer(5, "oneshot")
    test.socket.zwave:__queue_receive({
      mock_siren.id,
...
test.register_coroutine_test(
  "POWER_MANAGEMENT REPLACE_BATTERY_SOON on device without battery should trigger profile change",
  function()
    test.socket.zwave:__queue_receive({
      mock_siren.id,
      zw_test_utils.zwave_test_build_receive_command(
    ...
    mock_siren:expect_metadata_update({ profile = "aeotec-doorbell-siren-battery" })
  end
)
